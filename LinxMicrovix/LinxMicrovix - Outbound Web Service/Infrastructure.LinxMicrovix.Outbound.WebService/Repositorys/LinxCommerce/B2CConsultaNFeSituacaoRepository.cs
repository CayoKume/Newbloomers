    ...
    public class B2CConsultaNFeSituacaoRepository : IB2CConsultaNFeSituacaoRepository
    {
        ...
        public async Task<bool> CreateTableMerge(LinxAPIParam jobParameter)
        {
            string? sql = @"IF NOT EXISTS (SELECT * FROM SYS.OBJECTS WHERE TYPE = 'P' AND NAME = 'P_B2CCONSULTANFESITUACAO_SYNC')
                           ...
                           EXECUTE (
	                           ...
	                           BEGIN
		                           MERGE [B2CCONSULTANFESITUACAO] AS TARGET
                                   USING [B2CCONSULTANFESITUACAO] AS SOURCE
                                   ...
                                   WHEN NOT MATCHED BY TARGET AND SOURCE.[ID_NFE_SITUACAO] NOT IN (SELECT [ID_NFE_SITUACAO] FROM [B2CCONSULTANFESITUACAO]) THEN
			                           INSERT
        ...
        public async Task<List<B2CConsultaNFeSituacao>> GetRegistersExists(LinxAPIParam jobParameter, List<B2CConsultaNFeSituacao> registros)
        {
            try
            {
                ...
                string sql = $"SELECT ID_NFE_SITUACAO, TIMESTAMP FROM B2CCONSULTANFESITUACAO WHERE ID_NFE_SITUACAO IN ({identificadores})";
