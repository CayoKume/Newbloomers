...
namespace Infrastructure.LinxMicrovix.Outbound.WebService.Repository.LinxCommerce
{
    public class B2CConsultaSetoresRepository : IB2CConsultaSetoresRepository
    {
        ...
        {
            var table = _linxMicrovixRepositoryBase.CreateSystemDataTable(jobParameter.tableName, new B2CConsultaSetores());
        ...
        public async Task<IEnumerable<string?>> GetRegistersExists(LinxAPIParam jobParameter, List<B2CConsultaSetores> registros)
        {
            ...
            string sql = $"SELECT CONCAT('[', CODIGO_SETOR, ']', '|', '[', [TIMESTAMP], ']') FROM [linx_microvix_commerce].[B2CCONSULTASETORES] WHERE CODIGO_SETOR IN ({identificadores})";
