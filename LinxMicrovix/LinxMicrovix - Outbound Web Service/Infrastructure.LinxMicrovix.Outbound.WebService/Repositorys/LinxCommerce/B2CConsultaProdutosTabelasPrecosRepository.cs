...
namespace Infrastructure.LinxMicrovix.Outbound.WebService.Repository.LinxCommerce
{
    public class B2CConsultaProdutosTabelasPrecosRepository : IB2CConsultaProdutosTabelasPrecosRepository
    {
        ...
        public async Task<IEnumerable<string?>> GetRegistersExists(LinxAPIParam jobParameter, List<B2CConsultaProdutosTabelasPrecos> registros)
        {
            ...
            string sql = $"SELECT CONCAT('[', ID_PROD_TAB_PRECO, ']', '|', '[', [TIMESTAMP], ']') FROM [linx_microvix_commerce].[B2CConsultaProdutosTabelasPrecos] WHERE ID_PROD_TAB_PRECO IN ({identificadores})";
