    ...
    public class B2CConsultaProdutosCodebarService<TEntity> : IB2CConsultaProdutosCodebarService<TEntity> where TEntity : B2CConsultaProdutosCodebar, new()
    {
            ...
            for (int i = 0; i < records.Count(); i++)
            {
                ...
                catch (Exception ex)
                {
                    throw new InternalErrorException()
                    {
                        ...
                        message = $"Error when convert record: {records[i].Where(pair => pair.Key == "codigoproduto").Select(pair => pair.Value).FirstOrDefault()} - {records[i].Where(pair => pair.Key == "codebar").Select(pair => pair.Value).FirstOrDefault()}",
                        record = $"{records[i].Where(pair => pair.Key == "codigoproduto").Select(pair => pair.Value).FirstOrDefault()} - {records[i].Where(pair => pair.Key == "codebar").Select(pair => pair.Value).FirstOrDefault()}",
                        propertie = " - ",
        ...
        public async Task<bool> GetRecords(LinxMicrovixJobParameter jobParameter)
        {
            try
            {
                ...
                await _b2cConsultaProdutosCodebarRepository.CreateTableMerge(jobParameter: jobParameter);
                await _linxMicrovixRepositoryBase.ExecuteTruncateRawTable(jobParameter);
