...
{
    public class B2CConsultaClientesEstadoCivilService<TEntity> : IB2CConsultaClientesEstadoCivilService<TEntity> where TEntity : B2CConsultaClientesEstadoCivil, new()
    {
        private readonly IAPICall _apiCall;
        private readonly ILinxMicrovixServiceBase _linxMicrovixServiceBase;
        private readonly ILinxMicrovixRepositoryBase<TEntity> _linxMicrovixRepositoryBase;
        private readonly IB2CConsultaClientesEstadoCivilRepository<TEntity> _b2cConsultaClientesEstadoCivilRepository;

        public B2CConsultaClientesEstadoCivilService(
            IAPICall apiCall,
            ILinxMicrovixServiceBase linxMicrovixServiceBase,
            ILinxMicrovixRepositoryBase<TEntity> linxMicrovixRepositoryBase,
            IB2CConsultaClientesEstadoCivilRepository<TEntity> b2cConsultaClientesEstadoCivilRepository
        )
        {
            _apiCall = apiCall;
            ...
            for (int i = 0; i < records.Count(); i++)
            {
                try
                {
                    var entity = new B2CConsultaClientesEstadoCivil(
                        id_estado_civil: records[i].Where(pair => pair.Key == "id_estado_civil").Select(pair => pair.Value).First(),
                        estado_civil: records[i].Where(pair => pair.Key == "estado_civil").Select(pair => pair.Value).First(),
                ...
                catch (Exception ex)
                {
                    throw new InternalErrorException()
                    {
                        ...
                        message = $"Error when convert record: {records[i].Where(pair => pair.Key == "id_estado_civil").Select(pair => pair.Value).First()} - {records[i].Where(pair => pair.Key == "estado_civil").Select(pair => pair.Value).First()}",
                        record = $"{records[i].Where(pair => pair.Key == "id_estado_civil").Select(pair => pair.Value).First()} - {records[i].Where(pair => pair.Key == "estado_civil").Select(pair => pair.Value).First()}",
                        propertie = " - ",
        ...
        public async Task<bool> GetRecords(JobParameter jobParameter)
        {
            try
            {
                ...
                await _b2cConsultaClientesEstadoCivilRepository.ExecuteTableMerge(jobParameter: jobParameter);
                await _linxMicrovixRepositoryBase.ExecuteTruncateRawTable(jobParameter);
