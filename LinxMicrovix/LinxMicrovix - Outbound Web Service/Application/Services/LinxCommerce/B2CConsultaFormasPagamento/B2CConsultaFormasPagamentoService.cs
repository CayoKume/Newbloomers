    ...
    public class B2CConsultaFormasPagamentoService<TEntity> : IB2CConsultaFormasPagamentoService<TEntity> where TEntity : B2CConsultaFormasPagamento, new()
    {
            ...
            for (int i = 0; i < records.Count(); i++)
            {
                try
                {
                    var entity = new B2CConsultaFormasPagamento(
                        cod_forma_pgto: records[i].Where(pair => pair.Key == "cod_forma_pgto").Select(pair => pair.Value).First(),
                ...
                catch (Exception ex)
                {
                    throw new InternalErrorException()
                    {
                        ...
                        message = $"Error when convert record: {records[i].Where(pair => pair.Key == "cod_forma_pgto").Select(pair => pair.Value).First()} - {records[i].Where(pair => pair.Key == "forma_pgto").Select(pair => pair.Value).First()}",
                        record = $"{records[i].Where(pair => pair.Key == "cod_forma_pgto").Select(pair => pair.Value).First()} - {records[i].Where(pair => pair.Key == "forma_pgto").Select(pair => pair.Value).First()}",
                        propertie = " - ",
        ...
        public async Task<bool> GetRecords(JobParameter jobParameter)
        {
            try
            {
                ...
                await _b2cConsultaFormasPagamentoRepository.ExecuteTableMerge(jobParameter: jobParameter);
                await _linxMicrovixRepositoryBase.ExecuteTruncateRawTable(jobParameter);
