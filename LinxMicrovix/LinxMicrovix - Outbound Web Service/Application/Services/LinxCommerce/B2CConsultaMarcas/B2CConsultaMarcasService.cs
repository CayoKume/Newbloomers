...
{
    public class B2CConsultaMarcasService<TEntity> : IB2CConsultaMarcasService<TEntity> where TEntity : B2CConsultaMarcas, new()
    {
        private readonly IAPICall _apiCall;
        private readonly ILinxMicrovixServiceBase _linxMicrovixServiceBase;
        private readonly ILinxMicrovixRepositoryBase<TEntity> _linxMicrovixRepositoryBase;
        private readonly IB2CConsultaMarcasRepository<TEntity> _b2cConsultaMarcasRepository;

        public B2CConsultaMarcasService(
            IAPICall apiCall,
            ILinxMicrovixServiceBase linxMicrovixServiceBase,
            ILinxMicrovixRepositoryBase<TEntity> linxMicrovixRepositoryBase,
            IB2CConsultaMarcasRepository<TEntity> b2cConsultaMarcasRepository
        )
        {
            _apiCall = apiCall;
            ...
            for (int i = 0; i < records.Count(); i++)
            {
                try
                {
                    var entity = new B2CConsultaMarcas(
                        ...
                        nome_marca: records[i].Where(pair => pair.Key == "nome_marca").Select(pair => pair.Value).First(),
                        timestamp: records[i].Where(pair => pair.Key == "timestamp").Select(pair => pair.Value).First(),
                        linhas: records[i].Where(pair => pair.Key == "linhas").Select(pair => pair.Value).First(),
                ...
                catch (Exception ex)
                {
                    throw new InternalErrorException()
                    {
                        ...
                        message = $"Error when convert record: {records[i].Where(pair => pair.Key == "codigo_marca").Select(pair => pair.Value).First()} - {records[i].Where(pair => pair.Key == "nome_marca").Select(pair => pair.Value).First()}",
                        record = $"{records[i].Where(pair => pair.Key == "codigo_marca").Select(pair => pair.Value).First()} - {records[i].Where(pair => pair.Key == "nome_marca").Select(pair => pair.Value).First()}",
                        propertie = " - ",
        ...
        public async Task<bool> GetRecord(JobParameter jobParameter, string? identificador, string? cnpj_emp)
        {
            try
            {
                ...
                await _b2cConsultaMarcasRepository.InsertParametersIfNotExists(jobParameter);
        ...
        public async Task<bool> GetRecords(JobParameter jobParameter)
        {
            try
            {
                ...
                await _b2cConsultaMarcasRepository.ExecuteTableMerge(jobParameter: jobParameter);
                await _linxMicrovixRepositoryBase.ExecuteTruncateRawTable(jobParameter);
