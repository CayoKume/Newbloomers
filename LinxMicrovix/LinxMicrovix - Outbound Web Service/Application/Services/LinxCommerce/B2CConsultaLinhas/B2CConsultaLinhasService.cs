...
{
    public class B2CConsultaLinhasService<TEntity> : IB2CConsultaLinhasService<TEntity> where TEntity : B2CConsultaLinhas, new()
    {
        private readonly IAPICall _apiCall;
        private readonly ILinxMicrovixServiceBase _linxMicrovixServiceBase;
        private readonly ILinxMicrovixRepositoryBase<TEntity> _linxMicrovixRepositoryBase;
        private readonly IB2CConsultaLinhasRepository<TEntity> _b2cConsultaLinhasRepository;

        public B2CConsultaLinhasService(
            IAPICall apiCall,
            ILinxMicrovixServiceBase linxMicrovixServiceBase,
            ILinxMicrovixRepositoryBase<TEntity> linxMicrovixRepositoryBase,
            IB2CConsultaLinhasRepository<TEntity> b2cConsultaLinhasRepository
        )
        {
            _apiCall = apiCall;
            ...
            for (int i = 0; i < records.Count(); i++)
            {
                try
                {
                    var entity = new B2CConsultaLinhas(
                        ...
                        nome_linha: records[i].Where(pair => pair.Key == "nome_linha").Select(pair => pair.Value).First(),
                        timestamp: records[i].Where(pair => pair.Key == "timestamp").Select(pair => pair.Value).First(),
                        setores: records[i].Where(pair => pair.Key == "setores").Select(pair => pair.Value).First(),
                ...
                catch (Exception ex)
                {
                    throw new InternalErrorException()
                    {
                        ...
                        message = $"Error when convert record: {records[i].Where(pair => pair.Key == "codigo_linha").Select(pair => pair.Value).First()} - {records[i].Where(pair => pair.Key == "nome_linha").Select(pair => pair.Value).First()}",
                        record = $"{records[i].Where(pair => pair.Key == "codigo_linha").Select(pair => pair.Value).First()} - {records[i].Where(pair => pair.Key == "nome_linha").Select(pair => pair.Value).First()}",
                        propertie = " - ",
        ...
        public async Task<bool> GetRecords(JobParameter jobParameter)
        {
            try
            {
                ...
                await _b2cConsultaLinhasRepository.ExecuteTableMerge(jobParameter: jobParameter);
                await _linxMicrovixRepositoryBase.ExecuteTruncateRawTable(jobParameter);
