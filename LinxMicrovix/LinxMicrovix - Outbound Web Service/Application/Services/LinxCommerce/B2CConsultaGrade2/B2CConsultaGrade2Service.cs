...
{
    public class B2CConsultaGrade2Service<TEntity> : IB2CConsultaGrade2Service<TEntity> where TEntity : B2CConsultaGrade2, new()
    {
        private readonly IAPICall _apiCall;
        private readonly ILinxMicrovixServiceBase _linxMicrovixServiceBase;
        private readonly ILinxMicrovixRepositoryBase<TEntity> _linxMicrovixRepositoryBase;
        private readonly IB2CConsultaGrade2Repository<TEntity> _b2cConsultaGrade2Repository;

        public B2CConsultaGrade2Service(
            IAPICall apiCall,
            ILinxMicrovixServiceBase linxMicrovixServiceBase,
            ILinxMicrovixRepositoryBase<TEntity> linxMicrovixRepositoryBase,
            IB2CConsultaGrade2Repository<TEntity> b2cConsultaGrade2Repository
        )
        {
            _apiCall = apiCall;
            ...
            for (int i = 0; i < records.Count(); i++)
            {
                try
                {
                    var entity = new B2CConsultaGrade2(
                        ...
                        nome_grade2: records[i].Where(pair => pair.Key == "nome_grade2").Select(pair => pair.Value).First(),
        ...
        public async Task<bool> GetRecord(JobParameter jobParameter, string? identificador, string? cnpj_emp)
        {
            try
            {
                ...
                await _b2cConsultaGrade2Repository.InsertParametersIfNotExists(jobParameter);
        ...
        public async Task<bool> GetRecords(JobParameter jobParameter)
        {
            try
            {
                ...
                foreach (var cnpj_emp in cnpjs_emp)
                {
                    ...
                    {
                        var listRecords = DeserializeXMLToObject(jobParameter, xmls);
                        _b2cConsultaGrade2Repository.BulkInsertIntoTableRaw(records: listRecords, jobParameter: jobParameter);
                    }
                ...
                await _b2cConsultaGrade2Repository.ExecuteTableMerge(jobParameter: jobParameter);
                await _linxMicrovixRepositoryBase.ExecuteTruncateRawTable(jobParameter);
