...
{
    public class B2CConsultaProdutosTabelasService<TEntity> : IB2CConsultaProdutosTabelasService<TEntity> where TEntity : B2CConsultaProdutosTabelas, new()
    {
        private readonly IAPICall _apiCall;
        private readonly ILinxMicrovixServiceBase _linxMicrovixServiceBase;
        private readonly ILinxMicrovixRepositoryBase<TEntity> _linxMicrovixRepositoryBase;
        private readonly IB2CConsultaProdutosTabelasRepository<TEntity> _b2cConsultaProdutosTabelasRepository;

        public B2CConsultaProdutosTabelasService(
            IAPICall apiCall,
            ILinxMicrovixServiceBase linxMicrovixServiceBase,
            ILinxMicrovixRepositoryBase<TEntity> linxMicrovixRepositoryBase,
            IB2CConsultaProdutosTabelasRepository<TEntity> b2cConsultaProdutosTabelasRepository
        )
        {
            _apiCall = apiCall;
            ...
            for (int i = 0; i < records.Count(); i++)
            {
                try
                {
                    list.Add(new TEntity
                    {
                        ...
                        nome_tabela = records[i].Where(pair => pair.Key == "nome_tabela").Select(pair => pair.Value).First() == String.Empty ? ""
                        : records[i].Where(pair => pair.Key == "nome_tabela").Select(pair => pair.Value).First(),
                ...
                catch (Exception ex)
                {
                    throw new InternalErrorException()
                    {
                        ...
                        message = $"Error when convert record: {records[i].Where(pair => pair.Key == "id_tabela").Select(pair => pair.Value).First()} - {records[i].Where(pair => pair.Key == "nome_tabela").Select(pair => pair.Value).First()}",
                        record = $"{records[i].Where(pair => pair.Key == "id_tabela").Select(pair => pair.Value).First()} - {records[i].Where(pair => pair.Key == "nome_tabela").Select(pair => pair.Value).First()}",
                        propertie = " - ",
        ...
        public async Task<bool> GetRecords(JobParameter jobParameter)
        {
            try
            {
                ...
                await _b2cConsultaProdutosTabelasRepository.ExecuteTableMerge(jobParameter: jobParameter);
                await _linxMicrovixRepositoryBase.ExecuteTruncateRawTable(jobParameter);
