...
{
    public class B2CConsultaProdutosCamposAdicionaisNomesService<TEntity> : IB2CConsultaProdutosCamposAdicionaisNomesService<TEntity> where TEntity : B2CConsultaProdutosCamposAdicionaisNomes, new()
    {
        private readonly IAPICall _apiCall;
        private readonly ILinxMicrovixServiceBase _linxMicrovixServiceBase;
        private readonly ILinxMicrovixRepositoryBase<TEntity> _linxMicrovixRepositoryBase;
        private readonly IB2CConsultaProdutosCamposAdicionaisNomesRepository<TEntity> _b2cConsultaProdutosCamposAdicionaisNomesRepository;
        ...
        public B2CConsultaProdutosCamposAdicionaisNomesService(
            ...
            IB2CConsultaProdutosCamposAdicionaisNomesRepository<TEntity> b2cConsultaProdutosCamposAdicionaisNomesRepository
        )
        {
            _apiCall = apiCall;
            ...
            for (int i = 0; i < records.Count(); i++)
            {
                try
                {
                    var entity = new B2CConsultaProdutosCamposAdicionaisNomes(
                        ...
                        legenda: records[i].Where(pair => pair.Key == "legenda").Select(pair => pair.Value).First(),
                        timestamp: records[i].Where(pair => pair.Key == "timestamp").Select(pair => pair.Value).First(),
                        tipo: records[i].Where(pair => pair.Key == "tipo").Select(pair => pair.Value).First(),
                ...
                catch (Exception ex)
                {
                    throw new InternalErrorException()
                    {
                        ...
                        message = $"Error when convert record: {records[i].Where(pair => pair.Key == "id_campo").Select(pair => pair.Value).First()} - {records[i].Where(pair => pair.Key == "legenda").Select(pair => pair.Value).First()}",
                        record = $"{records[i].Where(pair => pair.Key == "id_campo").Select(pair => pair.Value).First()} - {records[i].Where(pair => pair.Key == "legenda").Select(pair => pair.Value).First()}",
                        propertie = " - ",
        ...
        public async Task<bool> GetRecord(JobParameter jobParameter, string? identificador, string? cnpj_emp)
        {
            try
            {
                ...
                await _b2cConsultaProdutosCamposAdicionaisNomesRepository.InsertParametersIfNotExists(jobParameter);
        ...
        public async Task<bool> GetRecords(JobParameter jobParameter)
        {
            try
            {
                ...
                await _b2cConsultaProdutosCamposAdicionaisNomesRepository.ExecuteTableMerge(jobParameter: jobParameter);
                await _linxMicrovixRepositoryBase.ExecuteTruncateRawTable(jobParameter);
