...
{
    public class B2CConsultaProdutosCamposAdicionaisValoresService<TEntity> : IB2CConsultaProdutosCamposAdicionaisValoresService<TEntity> where TEntity : B2CConsultaProdutosCamposAdicionaisValores, new()
    {
        private readonly IAPICall _apiCall;
        private readonly ILinxMicrovixServiceBase _linxMicrovixServiceBase;
        private readonly ILinxMicrovixRepositoryBase<TEntity> _linxMicrovixRepositoryBase;
        private readonly IB2CConsultaProdutosCamposAdicionaisValoresRepository<TEntity> _b2cConsultaProdutosCamposAdicionaisValoresRepository;
        ...
        public B2CConsultaProdutosCamposAdicionaisValoresService(
            ...
            IB2CConsultaProdutosCamposAdicionaisValoresRepository<TEntity> b2cConsultaProdutosCamposAdicionaisValoresRepository
        )
        {
            _apiCall = apiCall;
            ...
            for (int i = 0; i < records.Count(); i++)
            {
                try
                {
                    var entity = new B2CConsultaProdutosCamposAdicionaisValores(
                        id_campo_valor: records[i].Where(pair => pair.Key == "id_campo_valor").Select(pair => pair.Value).First(),
                ...
                catch (Exception ex)
                {
                    throw new InternalErrorException()
                    {
                        ...
                        message = $"Error when convert record: {records[i].Where(pair => pair.Key == "id_campo_valor").Select(pair => pair.Value).First()} - {records[i].Where(pair => pair.Key == "codigoproduto").Select(pair => pair.Value).First()}",
                        record = $"{records[i].Where(pair => pair.Key == "id_campo_valor").Select(pair => pair.Value).First()} - {records[i].Where(pair => pair.Key == "codigoproduto").Select(pair => pair.Value).First()}",
                        propertie = " - ",
        ...
        public async Task<bool> GetRecords(JobParameter jobParameter)
        {
            try
            {
                ...
                await _b2cConsultaProdutosCamposAdicionaisValoresRepository.ExecuteTableMerge(jobParameter: jobParameter);
                await _linxMicrovixRepositoryBase.ExecuteTruncateRawTable(jobParameter);
