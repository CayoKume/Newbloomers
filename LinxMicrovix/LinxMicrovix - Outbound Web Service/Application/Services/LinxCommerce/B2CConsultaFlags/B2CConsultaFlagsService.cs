    ...
    public class B2CConsultaFlagsService<TEntity> : IB2CConsultaFlagsService<TEntity> where TEntity : B2CConsultaFlags, new()
    {
            ...
            for (int i = 0; i < records.Count(); i++)
            {
                try
                {
                    var entity = new B2CConsultaFlags(
                        id_b2c_flags: records[i].Where(pair => pair.Key == "id_b2c_flags").Select(pair => pair.Value).First(),
                ...
                catch (Exception ex)
                {
                    throw new InternalErrorException()
                    {
                        ...
                        message = $"Error when convert record: {records[i].Where(pair => pair.Key == "id_b2c_flags").Select(pair => pair.Value).First()} - {records[i].Where(pair => pair.Key == "descricao").Select(pair => pair.Value).First()}",
                        record = $"{records[i].Where(pair => pair.Key == "id_b2c_flags").Select(pair => pair.Value).First()} - {records[i].Where(pair => pair.Key == "descricao").Select(pair => pair.Value).First()}",
                        propertie = " - ",
        ...
        public async Task<bool> GetRecords(JobParameter jobParameter)
        {
            try
            {
                ...
                await _b2cConsultaFlagsRepository.ExecuteTableMerge(jobParameter: jobParameter);
                await _linxMicrovixRepositoryBase.ExecuteTruncateRawTable(jobParameter);
