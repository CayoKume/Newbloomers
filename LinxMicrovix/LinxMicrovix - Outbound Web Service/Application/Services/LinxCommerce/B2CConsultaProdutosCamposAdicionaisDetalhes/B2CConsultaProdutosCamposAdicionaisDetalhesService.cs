...
{
    public class B2CConsultaProdutosCamposAdicionaisDetalhesService<TEntity> : IB2CConsultaProdutosCamposAdicionaisDetalhesService<TEntity> where TEntity : B2CConsultaProdutosCamposAdicionaisDetalhes, new()
    {
        private readonly IAPICall _apiCall;
        private readonly ILinxMicrovixServiceBase _linxMicrovixServiceBase;
        private readonly ILinxMicrovixRepositoryBase<TEntity> _linxMicrovixRepositoryBase;
        private readonly IB2CConsultaProdutosCamposAdicionaisDetalhesRepository<TEntity> _b2cConsultaProdutosCamposAdicionaisDetalhesRepository;
        ...
        public B2CConsultaProdutosCamposAdicionaisDetalhesService(
            ...
            IB2CConsultaProdutosCamposAdicionaisDetalhesRepository<TEntity> b2cConsultaProdutosCamposAdicionaisDetalhesRepository
        )
        {
            _apiCall = apiCall;
            ...
            for (int i = 0; i < records.Count(); i++)
            {
                try
                {
                    var entity = new B2CConsultaProdutosCamposAdicionaisDetalhes(
                        id_campo_detalhe: records[i].Where(pair => pair.Key == "id_campo_detalhe").Select(pair => pair.Value).First(),
                        ordem: records[i].Where(pair => pair.Key == "ordem").Select(pair => pair.Value).First(),
                        id_campo: records[i].Where(pair => pair.Key == "id_campo").Select(pair => pair.Value).First(),
                ...
                catch (Exception ex)
                {
                    throw new InternalErrorException()
                    {
                        ...
                        message = $"Error when convert record: {records[i].Where(pair => pair.Key == "id_campo_detalhe").Select(pair => pair.Value).First()} - {records[i].Where(pair => pair.Key == "descricao").Select(pair => pair.Value).First()}",
                        record = $"{records[i].Where(pair => pair.Key == "id_campo_detalhe").Select(pair => pair.Value).First()} - {records[i].Where(pair => pair.Key == "descricao").Select(pair => pair.Value).First()}",
                        propertie = " - ",
        ...
        public async Task<bool> GetRecord(JobParameter jobParameter, string? identificador, string? cnpj_emp)
        {
            try
            {
                ...
                await _b2cConsultaProdutosCamposAdicionaisDetalhesRepository.InsertParametersIfNotExists(jobParameter);
        ...
        public async Task<bool> GetRecords(JobParameter jobParameter)
        {
            try
            {
                ...
                await _b2cConsultaProdutosCamposAdicionaisDetalhesRepository.ExecuteTableMerge(jobParameter: jobParameter);
                await _linxMicrovixRepositoryBase.ExecuteTruncateRawTable(jobParameter);
