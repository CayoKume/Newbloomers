...
{
    public class B2CConsultaEspessurasService<TEntity> : IB2CConsultaEspessurasService<TEntity> where TEntity : B2CConsultaEspessuras, new()
    {
        private readonly IAPICall _apiCall;
        private readonly ILinxMicrovixServiceBase _linxMicrovixServiceBase;
        private readonly ILinxMicrovixRepositoryBase<TEntity> _linxMicrovixRepositoryBase;
        private readonly IB2CConsultaEspessurasRepository<TEntity> _b2cConsultaEspessurasRepository;

        public B2CConsultaEspessurasService(
            IAPICall apiCall,
            ILinxMicrovixServiceBase linxMicrovixServiceBase,
            ILinxMicrovixRepositoryBase<TEntity> linxMicrovixRepositoryBase,
            IB2CConsultaEspessurasRepository<TEntity> b2cConsultaEspessurasRepository
        )
        {
            _apiCall = apiCall;
            ...
            for (int i = 0; i < records.Count(); i++)
            {
                try
                {
                    var entity = new B2CConsultaEspessuras(
                        ...
                        nome_espessura: records[i].Where(pair => pair.Key == "nome_espessura").Select(pair => pair.Value).First(),
                ...
                catch (Exception ex)
                {
                    throw new InternalErrorException()
                    {
                        ...
                        message = $"Error when convert record: {records[i].Where(pair => pair.Key == "codigo_espessura").Select(pair => pair.Value).First()} - {records[i].Where(pair => pair.Key == "nome_espessura").Select(pair => pair.Value).First()}",
                        record = $"{records[i].Where(pair => pair.Key == "codigo_espessura").Select(pair => pair.Value).First()} - {records[i].Where(pair => pair.Key == "nome_espessura").Select(pair => pair.Value).First()}",
                        propertie = " - ",
        ...
        public async Task<bool> GetRecord(JobParameter jobParameter, string? identificador, string? cnpj_emp)
        {
            try
            {
                ...
                await _b2cConsultaEspessurasRepository.InsertParametersIfNotExists(jobParameter);
        ...
        public async Task<bool> GetRecords(JobParameter jobParameter)
        {
            try
            {
                ...
                await _b2cConsultaEspessurasRepository.ExecuteTableMerge(jobParameter: jobParameter);
                await _linxMicrovixRepositoryBase.ExecuteTruncateRawTable(jobParameter);
