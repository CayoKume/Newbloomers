...
{
    public class B2CConsultaPlanosParcelasService<TEntity> : IB2CConsultaPlanosParcelasService<TEntity> where TEntity : B2CConsultaPlanosParcelas, new()
    {
        private readonly IAPICall _apiCall;
        private readonly ILinxMicrovixServiceBase _linxMicrovixServiceBase;
        private readonly ILinxMicrovixRepositoryBase<TEntity> _linxMicrovixRepositoryBase;
        private readonly IB2CConsultaPlanosParcelasRepository<TEntity> _b2cConsultaPlanosParcelasRepository;

        public B2CConsultaPlanosParcelasService(
            IAPICall apiCall,
            ILinxMicrovixServiceBase linxMicrovixServiceBase,
            ILinxMicrovixRepositoryBase<TEntity> linxMicrovixRepositoryBase,
            IB2CConsultaPlanosParcelasRepository<TEntity> b2cConsultaPlanosParcelasRepository
        )
        {
            _apiCall = apiCall;
            ...
            for (int i = 0; i < records.Count(); i++)
            {
                try
                {
                    var entity = new B2CConsultaPlanosParcelas(
                        ...
                        ordem_parcela: records[i].Where(pair => pair.Key == "ordem_parcela").Select(pair => pair.Value).First(),
                        prazo_parc: records[i].Where(pair => pair.Key == "prazo_parc").Select(pair => pair.Value).First(),
                        id_planos_parcelas: records[i].Where(pair => pair.Key == "id_planos_parcelas").Select(pair => pair.Value).First(),
                ...
                catch (Exception ex)
                {
                    throw new InternalErrorException()
                    {
                        ...
                        message = $"Error when convert record: {records[i].Where(pair => pair.Key == "plano").Select(pair => pair.Value).First()} - {records[i].Where(pair => pair.Key == "id_planos_parcelas").Select(pair => pair.Value).First()}",
                        record = $"{records[i].Where(pair => pair.Key == "plano").Select(pair => pair.Value).First()} - {records[i].Where(pair => pair.Key == "id_planos_parcelas").Select(pair => pair.Value).First()}",
                        propertie = " - ",
        ...
        public async Task<bool> GetRecords(JobParameter jobParameter)
        {
            try
            {
                ...
                await _b2cConsultaPlanosParcelasRepository.ExecuteTableMerge(jobParameter: jobParameter);
                await _linxMicrovixRepositoryBase.ExecuteTruncateRawTable(jobParameter);
