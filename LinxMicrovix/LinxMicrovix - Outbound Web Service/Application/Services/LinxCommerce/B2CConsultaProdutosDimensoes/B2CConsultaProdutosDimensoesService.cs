...
{
    public class B2CConsultaProdutosDimensoesService<TEntity> : IB2CConsultaProdutosDimensoesService<TEntity> where TEntity : B2CConsultaProdutosDimensoes, new()
    {
        private readonly IAPICall _apiCall;
        private readonly ILinxMicrovixServiceBase _linxMicrovixServiceBase;
        private readonly ILinxMicrovixRepositoryBase<TEntity> _linxMicrovixRepositoryBase;
        private readonly IB2CConsultaProdutosDimensoesRepository<TEntity> _b2cConsultaProdutosDimensoesRepository;

        public B2CConsultaProdutosDimensoesService(
            IAPICall apiCall,
            ILinxMicrovixServiceBase linxMicrovixServiceBase,
            ILinxMicrovixRepositoryBase<TEntity> linxMicrovixRepositoryBase,
            IB2CConsultaProdutosDimensoesRepository<TEntity> b2cConsultaProdutosDimensoesRepository
        )
        {
            _apiCall = apiCall;
            ...
            for (int i = 0; i < records.Count(); i++)
            {
                try
                {
                    list.Add(new TEntity
                    {
                        ...
                        altura = records[i].Where(pair => pair.Key == "altura").Select(pair => pair.Value).First() == String.Empty ? 0
                        : Convert.ToDecimal(records[i].Where(pair => pair.Key == "altura").Select(pair => pair.Value).First()),
                        ...
                        comprimento = records[i].Where(pair => pair.Key == "comprimento").Select(pair => pair.Value).First() == String.Empty ? 0
                        : Convert.ToDecimal(records[i].Where(pair => pair.Key == "comprimento").Select(pair => pair.Value).First()),

                        largura = records[i].Where(pair => pair.Key == "largura").Select(pair => pair.Value).First() == String.Empty ? 0
                        : Convert.ToDecimal(records[i].Where(pair => pair.Key == "largura").Select(pair => pair.Value).First()),
                ...
                catch (Exception ex)
                {
                    throw new InternalErrorException()
                    {
                        ...
                        message = $"Error when convert record: {records[i].Where(pair => pair.Key == "codigoproduto").Select(pair => pair.Value).First()}",
                        record = $"{records[i].Where(pair => pair.Key == "codigoproduto").Select(pair => pair.Value).First()}",
                        propertie = " - ",
        ...
        public async Task<bool> GetRecord(JobParameter jobParameter, string? identificador, string? cnpj_emp)
        {
            try
            {
                ...
                await _b2cConsultaProdutosDimensoesRepository.InsertParametersIfNotExists(jobParameter);
        ...
        public async Task<bool> GetRecords(JobParameter jobParameter)
        {
            try
            {
                ...
                foreach (var cnpj_emp in cnpjs_emp)
                {
                    ...
                    {
                        var listRecords = DeserializeXMLToObject(jobParameter, xmls);
                        _b2cConsultaProdutosDimensoesRepository.BulkInsertIntoTableRaw(records: listRecords, jobParameter: jobParameter);
                    }
                ...
                await _b2cConsultaProdutosDimensoesRepository.ExecuteTableMerge(jobParameter: jobParameter);
                await _linxMicrovixRepositoryBase.ExecuteTruncateRawTable(jobParameter);
