...
{
    public class B2CConsultaTiposCobrancaFreteService<TEntity> : IB2CConsultaTiposCobrancaFreteService<TEntity> where TEntity : B2CConsultaTiposCobrancaFrete, new()
    {
        private readonly IAPICall _apiCall;
        private readonly ILinxMicrovixServiceBase _linxMicrovixServiceBase;
        private readonly ILinxMicrovixRepositoryBase<TEntity> _linxMicrovixRepositoryBase;
        private readonly IB2CConsultaTiposCobrancaFreteRepository<TEntity> _b2cConsultaTiposCobrancaFreteRepository;

        public B2CConsultaTiposCobrancaFreteService(
            IAPICall apiCall,
            ILinxMicrovixServiceBase linxMicrovixServiceBase,
            ILinxMicrovixRepositoryBase<TEntity> linxMicrovixRepositoryBase,
            IB2CConsultaTiposCobrancaFreteRepository<TEntity> b2cConsultaTiposCobrancaFreteRepository
        )
        {
            _apiCall = apiCall;
            ...
            for (int i = 0; i < records.Count(); i++)
            {
                try
                {
                    list.Add(new TEntity
                    {
                        ...
                        codigo_tipo_cobranca_frete = records[i].Where(pair => pair.Key == "codigo_tipo_cobranca_frete").Select(pair => pair.Value).First() == String.Empty ? 0
                        : Convert.ToInt32(records[i].Where(pair => pair.Key == "codigo_tipo_cobranca_frete").Select(pair => pair.Value).First()),

                        nome_tipo_cobranca_frete = records[i].Where(pair => pair.Key == "nome_tipo_cobranca_frete").Select(pair => pair.Value).First() == String.Empty ? ""
                        : records[i].Where(pair => pair.Key == "nome_tipo_cobranca_frete").Select(pair => pair.Value).First(),
                ...
                catch (Exception ex)
                {
                    throw new InternalErrorException()
                    {
                        ...
                        message = $"Error when convert record: {records[i].Where(pair => pair.Key == "codigo_tipo_cobranca_frete").Select(pair => pair.Value).First()} - {records[i].Where(pair => pair.Key == "nome_tipo_cobranca_frete").Select(pair => pair.Value).First()}",
                        record = $"{records[i].Where(pair => pair.Key == "codigo_tipo_cobranca_frete").Select(pair => pair.Value).First()} - {records[i].Where(pair => pair.Key == "nome_tipo_cobranca_frete").Select(pair => pair.Value).First()}",
                        propertie = " - ",
        ...
        public async Task<bool> GetRecords(JobParameter jobParameter)
        {
            try
            {
                ...
                await _b2cConsultaTiposCobrancaFreteRepository.ExecuteTableMerge(jobParameter: jobParameter);
                await _linxMicrovixRepositoryBase.ExecuteTruncateRawTable(jobParameter);
