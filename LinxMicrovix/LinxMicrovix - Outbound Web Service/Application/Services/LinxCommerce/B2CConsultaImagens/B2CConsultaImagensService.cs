...
{
    public class B2CConsultaImagensService<TEntity> : IB2CConsultaImagensService<TEntity> where TEntity : B2CConsultaImagens, new()
    {
        private readonly IAPICall _apiCall;
        private readonly ILinxMicrovixServiceBase _linxMicrovixServiceBase;
        private readonly ILinxMicrovixRepositoryBase<TEntity> _linxMicrovixRepositoryBase;
        private readonly IB2CConsultaImagensRepository<TEntity> _b2cConsultaImagensRepository;

        public B2CConsultaImagensService(
            IAPICall apiCall,
            ILinxMicrovixServiceBase linxMicrovixServiceBase,
            ILinxMicrovixRepositoryBase<TEntity> linxMicrovixRepositoryBase,
            IB2CConsultaImagensRepository<TEntity> b2cConsultaImagensRepository
        )
        {
            _apiCall = apiCall;
            ...
            for (int i = 0; i < records.Count(); i++)
            {
                try
                {
                    var entity = new B2CConsultaImagens(
                        id_imagem: records[i].Where(pair => pair.Key == "id_imagem").Select(pair => pair.Value).First(),
                        imagem: records[i].Where(pair => pair.Key == "imagem").Select(pair => pair.Value).First(),
                        md5: records[i].Where(pair => pair.Key == "md5").Select(pair => pair.Value).First(),
                        url_imagem_blob: records[i].Where(pair => pair.Key == "url_imagem_blob").Select(pair => pair.Value).First(),
                ...
                catch (Exception ex)
                {
                    throw new InternalErrorException()
                    {
                        ...
                        message = $"Error when convert record: {records[i].Where(pair => pair.Key == "id_imagem").Select(pair => pair.Value).First()} - {records[i].Where(pair => pair.Key == "imagem").Select(pair => pair.Value).First()}",
                        record = $"{records[i].Where(pair => pair.Key == "id_imagem").Select(pair => pair.Value).First()} - {records[i].Where(pair => pair.Key == "imagem").Select(pair => pair.Value).First()}",
                        propertie = " - ",
        ...
        public async Task<bool> GetRecords(JobParameter jobParameter)
        {
            try
            {
                ...
                await _b2cConsultaImagensRepository.ExecuteTableMerge(jobParameter: jobParameter);
                await _linxMicrovixRepositoryBase.ExecuteTruncateRawTable(jobParameter);
