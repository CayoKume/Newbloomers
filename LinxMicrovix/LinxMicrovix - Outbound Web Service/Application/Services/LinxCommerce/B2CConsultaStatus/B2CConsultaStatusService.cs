    ...
    public class B2CConsultaStatusService<TEntity> : IB2CConsultaStatusService<TEntity> where TEntity : B2CConsultaStatus, new()
    {
            ...
            for (int i = 0; i < records.Count(); i++)
            {
                try
                {
                    list.Add(new TEntity
                    {
                        ...
                        id_status = records[i].Where(pair => pair.Key == "id_status").Select(pair => pair.Value).First() == String.Empty ? 0
                        : Convert.ToInt32(records[i].Where(pair => pair.Key == "id_status").Select(pair => pair.Value).First()),

                        descricao_status = records[i].Where(pair => pair.Key == "descricao_status").Select(pair => pair.Value).First() == String.Empty ? ""
                        : records[i].Where(pair => pair.Key == "descricao_status").Select(pair => pair.Value).First(),
                ...
                catch (Exception ex)
                {
                    throw new InternalErrorException()
                    {
                        ...
                        message = $"Error when convert record: {records[i].Where(pair => pair.Key == "id_status").Select(pair => pair.Value).First()} - {records[i].Where(pair => pair.Key == "descricao_status").Select(pair => pair.Value).First()}",
                        record = $"{records[i].Where(pair => pair.Key == "id_status").Select(pair => pair.Value).First()} - {records[i].Where(pair => pair.Key == "descricao_status").Select(pair => pair.Value).First()}",
                        propertie = " - ",
        ...
        public async Task<bool> GetRecords(JobParameter jobParameter)
        {
            try
            {
                ...
                await _b2cConsultaStatusRepository.ExecuteTableMerge(jobParameter: jobParameter);
                await _linxMicrovixRepositoryBase.ExecuteTruncateRawTable(jobParameter);
