...
{
    public class B2CConsultaColecoesService<TEntity> : IB2CConsultaColecoesService<TEntity> where TEntity : B2CConsultaColecoes, new()
    {
        private readonly IAPICall _apiCall;
        private readonly ILinxMicrovixServiceBase _linxMicrovixServiceBase;
        private readonly ILinxMicrovixRepositoryBase<TEntity> _linxMicrovixRepositoryBase;
        private readonly IB2CConsultaColecoesRepository<TEntity> _b2cConsultaColecoesRepository;

        public B2CConsultaColecoesService(
            IAPICall apiCall,
            ILinxMicrovixServiceBase linxMicrovixServiceBase,
            ILinxMicrovixRepositoryBase<TEntity> linxMicrovixRepositoryBase,
            IB2CConsultaColecoesRepository<TEntity> b2cConsultaColecoesRepository
        )
        {
            _apiCall = apiCall;
            ...
            for (int i = 0; i < records.Count(); i++)
            {
                try
                {
                    var entity = new B2CConsultaColecoes(
                        ...
                        marcas: records[i].Where(pair => pair.Key == "marcas").Select(pair => pair.Value).First(),
                ...
                catch (Exception ex)
                {
                    throw new InternalErrorException()
                    {
                        ...
                        message = $"Error when convert record: {records[i].Where(pair => pair.Key == "codigo_colecao").Select(pair => pair.Value).First()} - {records[i].Where(pair => pair.Key == "nome_colecao").Select(pair => pair.Value).First()}",
                        record = $"{records[i].Where(pair => pair.Key == "codigo_colecao").Select(pair => pair.Value).First()} - {records[i].Where(pair => pair.Key == "nome_colecao").Select(pair => pair.Value).First()}",
                        propertie = " - ",
        ...
        public async Task<bool> GetRecords(JobParameter jobParameter)
        {
            try
            {
                ...
                await _b2cConsultaColecoesRepository.ExecuteTableMerge(jobParameter: jobParameter);
                await _linxMicrovixRepositoryBase.ExecuteTruncateRawTable(jobParameter);
