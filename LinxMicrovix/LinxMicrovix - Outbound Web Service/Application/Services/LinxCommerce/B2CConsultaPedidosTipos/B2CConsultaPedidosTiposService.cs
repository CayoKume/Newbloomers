...
{
    public class B2CConsultaPedidosTiposService<TEntity> : IB2CConsultaPedidosTiposService<TEntity> where TEntity : B2CConsultaPedidosTipos, new()
    {
        private readonly IAPICall _apiCall;
        private readonly ILinxMicrovixServiceBase _linxMicrovixServiceBase;
        private readonly ILinxMicrovixRepositoryBase<TEntity> _linxMicrovixRepositoryBase;
        private readonly IB2CConsultaPedidosTiposRepository<TEntity> _b2cConsultaPedidosTiposRepository;

        public B2CConsultaPedidosTiposService(
            IAPICall apiCall,
            ILinxMicrovixServiceBase linxMicrovixServiceBase,
            ILinxMicrovixRepositoryBase<TEntity> linxMicrovixRepositoryBase,
            IB2CConsultaPedidosTiposRepository<TEntity> b2cConsultaPedidosTiposRepository
        )
        {
            _apiCall = apiCall;
            ...
            for (int i = 0; i < records.Count(); i++)
            {
                try
                {
                    var entity = new B2CConsultaPedidosTipos(
                        ...
                        pos_timestamp_old: records[i].Where(pair => pair.Key == "pos_timestamp_old").Select(pair => pair.Value).First(),
                ...
                catch (Exception ex)
                {
                    throw new InternalErrorException()
                    {
                        ...
                        message = $"Error when convert record: {records[i].Where(pair => pair.Key == "id_tipo_b2c").Select(pair => pair.Value).First()} - {records[i].Where(pair => pair.Key == "descricao").Select(pair => pair.Value).First()}",
                        record = $"{records[i].Where(pair => pair.Key == "id_tipo_b2c").Select(pair => pair.Value).First()} - {records[i].Where(pair => pair.Key == "descricao").Select(pair => pair.Value).First()}",
                        propertie = " - ",
        ...
        public async Task<bool> GetRecords(JobParameter jobParameter)
        {
            try
            {
                ...
                await _b2cConsultaPedidosTiposRepository.ExecuteTableMerge(jobParameter: jobParameter);
                await _linxMicrovixRepositoryBase.ExecuteTruncateRawTable(jobParameter);
