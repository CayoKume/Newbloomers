    ...
    public class B2CConsultaClientesSaldoService<TEntity> : IB2CConsultaClientesSaldoService<TEntity> where TEntity : B2CConsultaClientesSaldo, new()
    {
            ...
            for (int i = 0; i < records.Count(); i++)
            {
                try
                {
                    var entity = new B2CConsultaClientesSaldo(
                        saldo: records[i].Where(pair => pair.Key == "saldo").Select(pair => pair.Value).First(),
                        cod_cliente_erp: records[i].Where(pair => pair.Key == "cod_cliente_erp").Select(pair => pair.Value).First(),
                        empresa: records[i].Where(pair => pair.Key == "saldo").Select(pair => pair.Value).First(),
                ...
                catch (Exception ex)
                {
                    throw new InternalErrorException()
                    {
                        ...
                        message = $"Error when convert record: Saldo - {records[i].Where(pair => pair.Key == "saldo").Select(pair => pair.Value).First()} Cliente - {records[i].Where(pair => pair.Key == "cod_cliente_erp").Select(pair => pair.Value).First()}",
                        record = $"Saldo - {records[i].Where(pair => pair.Key == "saldo").Select(pair => pair.Value).First()} Cliente - {records[i].Where(pair => pair.Key == "cod_cliente_erp").Select(pair => pair.Value).First()}",
                        propertie = " - ",
        ...
        public async Task<bool> GetRecords(JobParameter jobParameter)
        {
            try
            {
                ...
                await _b2cConsultaClientesSaldoRepository.ExecuteTableMerge(jobParameter: jobParameter);
                await _linxMicrovixRepositoryBase.ExecuteTruncateRawTable(jobParameter);
