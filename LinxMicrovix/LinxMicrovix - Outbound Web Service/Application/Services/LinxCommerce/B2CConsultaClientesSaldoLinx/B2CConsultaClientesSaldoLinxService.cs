...
{
    public class B2CConsultaClientesSaldoLinxService<TEntity> : IB2CConsultaClientesSaldoLinxService<TEntity> where TEntity : B2CConsultaClientesSaldoLinx, new()
    {
        private readonly IAPICall _apiCall;
        private readonly ILinxMicrovixServiceBase _linxMicrovixServiceBase;
        private readonly ILinxMicrovixRepositoryBase<TEntity> _linxMicrovixRepositoryBase;
        private readonly IB2CConsultaClientesSaldoLinxRepository<TEntity> _b2cConsultaClientesSaldoLinxRepository;

        public B2CConsultaClientesSaldoLinxService(
            IAPICall apiCall,
            IB2CConsultaClientesSaldoLinxRepository<TEntity> b2cConsultaClientesSaldoLinxRepository,
            ...
            for (int i = 0; i < records.Count(); i++)
            {
                try
                {
                    var entity = new B2CConsultaClientesSaldoLinx(
                        ...
                        valor: records[i].Where(pair => pair.Key == "valor").Select(pair => pair.Value).First(),
                ...
                catch (Exception ex)
                {
                    throw new InternalErrorException()
                    {
                        ...
                        message = $"Error when convert record: Saldo - {records[i].Where(pair => pair.Key == "saldo").Select(pair => pair.Value).First()} Cliente - {records[i].Where(pair => pair.Key == "cod_cliente_b2c").Select(pair => pair.Value).First()}",
                        record = $"Saldo - {records[i].Where(pair => pair.Key == "saldo").Select(pair => pair.Value).First()} Cliente - {records[i].Where(pair => pair.Key == "cod_cliente_b2c").Select(pair => pair.Value).First()}",
                        propertie = " - ",
        ...
        public async Task<bool> GetRecords(JobParameter jobParameter)
        {
            try
            {
                ...
                await _b2cConsultaClientesSaldoLinxRepository.ExecuteTableMerge(jobParameter: jobParameter);
                await _linxMicrovixRepositoryBase.ExecuteTruncateRawTable(jobParameter);
