...
namespace LinxMicrovix_Outbound_Web_Service.Application.Services.LinxCommerce
{
    public class B2CConsultaNFeSituacaoService<TEntity> : IB2CConsultaNFeSituacaoService<TEntity> where TEntity : B2CConsultaNFeSituacao, new()
    {
        ...
        private readonly IB2CConsultaNFeSituacaoRepository<TEntity> _b2cConsultaNFeSituacaoRepository;

        public B2CConsultaNFeSituacaoService(
            IAPICall apiCall,
            ILinxMicrovixServiceBase linxMicrovixServiceBase,
            ILinxMicrovixRepositoryBase<TEntity> linxMicrovixRepositoryBase,
            IB2CConsultaNFeSituacaoRepository<TEntity> b2cConsultaNFeSituacaoRepository
        )
        {
            _apiCall = apiCall;
            ...
            for (int i = 0; i < records.Count(); i++)
            {
                try
                {
                    var entity = new B2CConsultaNFeSituacao(
                        id_nfe_situacao: records[i].Where(pair => pair.Key == "id_nfe_situacao").Select(pair => pair.Value).First(),
                ...
                catch (Exception ex)
                {
                    throw new InternalErrorException()
                    {
                        ...
                        message = $"Error when convert record: {records[i].Where(pair => pair.Key == "id_nfe_situacao").Select(pair => pair.Value).First()} - {records[i].Where(pair => pair.Key == "descricao").Select(pair => pair.Value).First()}",
                        record = $"{records[i].Where(pair => pair.Key == "id_nfe_situacao").Select(pair => pair.Value).First()} - {records[i].Where(pair => pair.Key == "descricao").Select(pair => pair.Value).First()}",
                        propertie = " - ",
        ...
        public async Task<bool> GetRecords(JobParameter jobParameter)
        {
            try
            {
                ...
                await _b2cConsultaNFeSituacaoRepository.InsertParametersIfNotExists(jobParameter);
                ...
                await _b2cConsultaNFeSituacaoRepository.ExecuteTableMerge(jobParameter: jobParameter);
                await _linxMicrovixRepositoryBase.ExecuteTruncateRawTable(jobParameter);
