...
{
    public class B2CConsultaClientesEnderecosEntregaService<TEntity> : IB2CConsultaClientesEnderecosEntregaService<TEntity> where TEntity : B2CConsultaClientesEnderecosEntrega, new()
    {
        private readonly IAPICall _apiCall;
        private readonly ILinxMicrovixServiceBase _linxMicrovixServiceBase;
        private readonly ILinxMicrovixRepositoryBase<TEntity> _linxMicrovixRepositoryBase;
        private readonly IB2CConsultaClientesEnderecosEntregaRepository<TEntity> _b2cConsultaClientesEnderecosEntregaRepository;

        public B2CConsultaClientesEnderecosEntregaService(
            IAPICall apiCall,
            ILinxMicrovixServiceBase linxMicrovixServiceBase,
            ILinxMicrovixRepositoryBase<TEntity> linxMicrovixRepositoryBase,
            IB2CConsultaClientesEnderecosEntregaRepository<TEntity> b2cConsultaClientesEnderecosEntregaRepository
        )
        {
            _apiCall = apiCall;
            ...
            for (int i = 0; i < records.Count(); i++)
            {
                try
                {
                    var entity = new B2CConsultaClientesEnderecosEntrega(
                        id_endereco_entrega: records[i].Where(pair => pair.Key == "id_endereco_entrega").Select(pair => pair.Value).First(),
                        ...
                        endereco_cliente: records[i].Where(pair => pair.Key == "endereco_cliente").Select(pair => pair.Value).First(),
                        numero_rua_cliente: records[i].Where(pair => pair.Key == "numero_rua_cliente").Select(pair => pair.Value).First(),
                        complemento_end_cli: records[i].Where(pair => pair.Key == "complemento_end_cli").Select(pair => pair.Value).First(),
                        ...
                        descricao: records[i].Where(pair => pair.Key == "descricao").Select(pair => pair.Value).First(),
                        principal: records[i].Where(pair => pair.Key == "principal").Select(pair => pair.Value).First(),
                        id_cidade: records[i].Where(pair => pair.Key == "id_cidade").Select(pair => pair.Value).First(),
                ...
                catch (Exception ex)
                {
                    throw new InternalErrorException()
                    {
                        ...
                        message = $"Error when convert record: {records[i].Where(pair => pair.Key == "id_endereco_entrega").Select(pair => pair.Value).First()} - {records[i].Where(pair => pair.Key == "endereco_cliente").Select(pair => pair.Value).First()}",
                        record = $"{records[i].Where(pair => pair.Key == "id_endereco_entrega").Select(pair => pair.Value).First()} - {records[i].Where(pair => pair.Key == "endereco_cliente").Select(pair => pair.Value).First()}",
                        propertie = " - ",
        ...
        public async Task<bool> GetRecord(JobParameter jobParameter, string? identificador, string? cnpj_emp)
        {
            try
            {
                ...
                await _b2cConsultaClientesEnderecosEntregaRepository.InsertParametersIfNotExists(jobParameter);
        ...
        public async Task<bool> GetRecords(JobParameter jobParameter)
        {
            try
            {
                ...
                await _b2cConsultaClientesEnderecosEntregaRepository.ExecuteTableMerge(jobParameter: jobParameter);
                await _linxMicrovixRepositoryBase.ExecuteTruncateRawTable(jobParameter);
