...
{
    public class B2CConsultaUnidadeService<TEntity> : IB2CConsultaUnidadeService<TEntity> where TEntity : B2CConsultaUnidade, new()
    {
        private readonly IAPICall _apiCall;
        private readonly ILinxMicrovixServiceBase _linxMicrovixServiceBase;
        private readonly ILinxMicrovixRepositoryBase<TEntity> _linxMicrovixRepositoryBase;
        private readonly IB2CConsultaUnidadeRepository<TEntity> _b2cConsultaUnidadeRepository;

        public B2CConsultaUnidadeService(
            IAPICall apiCall,
            ILinxMicrovixServiceBase linxMicrovixServiceBase,
            ILinxMicrovixRepositoryBase<TEntity> linxMicrovixRepositoryBase,
            IB2CConsultaUnidadeRepository<TEntity> b2cConsultaUnidadeRepository
        )
        {
            _apiCall = apiCall;
            ...
            for (int i = 0; i < records.Count(); i++)
            {
                try
                {
                    list.Add(new TEntity
                    {
                        ...
                        unidade = records[i].Where(pair => pair.Key == "unidade").Select(pair => pair.Value).First() == String.Empty ? ""
                        : records[i].Where(pair => pair.Key == "unidade").Select(pair => pair.Value).First(),
                ...
                catch (Exception ex)
                {
                    throw new InternalErrorException()
                    {
                        ...
                        message = $"Error when convert record: {records[i].Where(pair => pair.Key == "unidade").Select(pair => pair.Value).First()}",
                        record = $"{records[i].Where(pair => pair.Key == "unidade").Select(pair => pair.Value).First()}",
                        propertie = " - ",
        ...
        public async Task<bool> GetRecord(JobParameter jobParameter, string? identificador, string? cnpj_emp)
        {
            try
            {
                ...
                await _b2cConsultaUnidadeRepository.InsertParametersIfNotExists(jobParameter);
        ...
        public async Task<bool> GetRecords(JobParameter jobParameter)
        {
            try
            {
                ...
                foreach (var cnpj_emp in cnpjs_emp)
                {
                    ...
                    {
                        var listRecords = DeserializeXMLToObject(jobParameter, xmls);
                        _b2cConsultaUnidadeRepository.BulkInsertIntoTableRaw(records: listRecords, jobParameter: jobParameter);
                    }
                ...
                await _b2cConsultaUnidadeRepository.ExecuteTableMerge(jobParameter: jobParameter);
                await _linxMicrovixRepositoryBase.ExecuteTruncateRawTable(jobParameter);
