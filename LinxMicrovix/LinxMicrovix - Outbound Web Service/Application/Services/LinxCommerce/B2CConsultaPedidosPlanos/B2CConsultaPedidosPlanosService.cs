...
{
    public class B2CConsultaPedidosPlanosService<TEntity> : IB2CConsultaPedidosPlanosService<TEntity> where TEntity : B2CConsultaPedidosPlanos, new()
    {
        private readonly IAPICall _apiCall;
        private readonly ILinxMicrovixServiceBase _linxMicrovixServiceBase;
        private readonly ILinxMicrovixRepositoryBase<TEntity> _linxMicrovixRepositoryBase;
        private readonly IB2CConsultaPedidosPlanosRepository<TEntity> _b2cConsultaPedidosPlanosRepository;

        public B2CConsultaPedidosPlanosService(
            IAPICall apiCall,
            ILinxMicrovixServiceBase linxMicrovixServiceBase,
            ILinxMicrovixRepositoryBase<TEntity> linxMicrovixRepositoryBase,
            IB2CConsultaPedidosPlanosRepository<TEntity> b2cConsultaPedidosPlanosRepository
        )
        {
            _apiCall = apiCall;
            ...
            for (int i = 0; i < records.Count(); i++)
            {
                try
                {
                    var entity = new B2CConsultaPedidosPlanos(
                        id_pedido_planos: records[i].Where(pair => pair.Key == "id_pedido_planos").Select(pair => pair.Value).First(),
                        ...
                        valor_plano: records[i].Where(pair => pair.Key == "valor_plano").Select(pair => pair.Value).First(),
                        timestamp: records[i].Where(pair => pair.Key == "timestamp").Select(pair => pair.Value).First(),
                        nsu_sitef: records[i].Where(pair => pair.Key == "nsu_sitef").Select(pair => pair.Value).First(),
                        cod_autorizacao: records[i].Where(pair => pair.Key == "cod_autorizacao").Select(pair => pair.Value).First(),
                        texto_comprovante: records[i].Where(pair => pair.Key == "texto_comprovante").Select(pair => pair.Value).First(),
                        cod_loja_sitef: records[i].Where(pair => pair.Key == "cod_loja_sitef").Select(pair => pair.Value).First(),
                ...
                catch (Exception ex)
                {
                    throw new InternalErrorException()
                    {
                        ...
                        message = $"Error when convert record: {records[i].Where(pair => pair.Key == "id_pedido_planos").Select(pair => pair.Value).First()} - {records[i].Where(pair => pair.Key == "plano_pagamento").Select(pair => pair.Value).First()}",
                        record = $"{records[i].Where(pair => pair.Key == "id_pedido_planos").Select(pair => pair.Value).First()} - {records[i].Where(pair => pair.Key == "plano_pagamento").Select(pair => pair.Value).First()}",
                        propertie = " - ",
        ...
        public async Task<bool> GetRecords(JobParameter jobParameter)
        {
            try
            {
                ...
                await _b2cConsultaPedidosPlanosRepository.ExecuteTableMerge(jobParameter: jobParameter);
                await _linxMicrovixRepositoryBase.ExecuteTruncateRawTable(jobParameter);
