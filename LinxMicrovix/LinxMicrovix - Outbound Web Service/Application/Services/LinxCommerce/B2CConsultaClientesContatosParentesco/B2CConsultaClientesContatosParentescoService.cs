...
{
    public class B2CConsultaClientesContatosParentescoService<TEntity> : IB2CConsultaClientesContatosParentescoService<TEntity> where TEntity : B2CConsultaClientesContatosParentesco, new()
    {
        private readonly IAPICall _apiCall;
        private readonly ILinxMicrovixServiceBase _linxMicrovixServiceBase;
        private readonly ILinxMicrovixRepositoryBase<TEntity> _linxMicrovixRepositoryBase;
        private readonly IB2CConsultaClientesContatosParentescoRepository<TEntity> _b2cConsultaClientesContatosParentescoRepository;
        ...
        public B2CConsultaClientesContatosParentescoService(
            ...
            IB2CConsultaClientesContatosParentescoRepository<TEntity> b2cConsultaClientesContatosParentescoRepository
        )
        {
            _apiCall = apiCall;
            ...
            for (int i = 0; i < records.Count(); i++)
            {
                try
                {
                    var entity = new B2CConsultaClientesContatosParentesco(
                        ...
                        descricao_parentesco: records[i].Where(pair => pair.Key == "descricao_parentesco").Select(pair => pair.Value).First(),
                ...
                catch (Exception ex)
                {
                    throw new InternalErrorException()
                    {
                        ...
                        message = $"Error when convert record: {records[i].Where(pair => pair.Key == "id_parentesco").Select(pair => pair.Value).First()} - {records[i].Where(pair => pair.Key == "descricao_parentesco").Select(pair => pair.Value).First()}",
                        record = $"{records[i].Where(pair => pair.Key == "id_parentesco").Select(pair => pair.Value).First()} - {records[i].Where(pair => pair.Key == "descricao_parentesco").Select(pair => pair.Value).First()}",
                        propertie = " - ",
        ...
        public async Task<bool> GetRecords(JobParameter jobParameter)
        {
            try
            {
                ...
                await _b2cConsultaClientesContatosParentescoRepository.ExecuteTableMerge(jobParameter: jobParameter);
                await _linxMicrovixRepositoryBase.ExecuteTruncateRawTable(jobParameter);
