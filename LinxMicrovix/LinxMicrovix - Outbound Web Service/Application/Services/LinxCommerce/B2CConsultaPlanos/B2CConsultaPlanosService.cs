...
{
    public class B2CConsultaPlanosService<TEntity> : IB2CConsultaPlanosService<TEntity> where TEntity : B2CConsultaPlanos, new()
    {
        private readonly IAPICall _apiCall;
        private readonly ILinxMicrovixServiceBase _linxMicrovixServiceBase;
        private readonly ILinxMicrovixRepositoryBase<TEntity> _linxMicrovixRepositoryBase;
        private readonly IB2CConsultaPlanosRepository<TEntity> _b2cConsultaPlanosRepository;

        public B2CConsultaPlanosService(
            IAPICall apiCall,
            ILinxMicrovixServiceBase linxMicrovixServiceBase,
            ILinxMicrovixRepositoryBase<TEntity> linxMicrovixRepositoryBase,
            IB2CConsultaPlanosRepository<TEntity> b2cConsultaPlanosRepository
        )
        {
            _apiCall = apiCall;
            ...
            for (int i = 0; i < records.Count(); i++)
            {
                try
                {
                    var entity = new B2CConsultaPlanos(
                        plano: records[i].Where(pair => pair.Key == "plano").Select(pair => pair.Value).First(),
                        nome_plano: records[i].Where(pair => pair.Key == "nome_plano").Select(pair => pair.Value).First(),
                        forma_pagamento: records[i].Where(pair => pair.Key == "forma_pagamento").Select(pair => pair.Value).First(),
                        qtde_parcelas: records[i].Where(pair => pair.Key == "qtde_parcelas").Select(pair => pair.Value).First(),
                        valor_minimo_parcela: records[i].Where(pair => pair.Key == "valor_minimo_parcela").Select(pair => pair.Value).First(),
                        indice: records[i].Where(pair => pair.Key == "indice").Select(pair => pair.Value).First(),
                        timestamp: records[i].Where(pair => pair.Key == "timestamp").Select(pair => pair.Value).First(),
                        desativado: records[i].Where(pair => pair.Key == "desativado").Select(pair => pair.Value).First(),
                        tipo_plano: records[i].Where(pair => pair.Key == "tipo_plano").Select(pair => pair.Value).First(),
                ...
                catch (Exception ex)
                {
                    throw new InternalErrorException()
                    {
                        ...
                        message = $"Error when convert record: {records[i].Where(pair => pair.Key == "plano").Select(pair => pair.Value).First()} - {records[i].Where(pair => pair.Key == "nome_plano").Select(pair => pair.Value).First()}",
                        record = $"{records[i].Where(pair => pair.Key == "plano").Select(pair => pair.Value).First()} - {records[i].Where(pair => pair.Key == "nome_plano").Select(pair => pair.Value).First()}",
                        propertie = " - ",
        ...
        public async Task<bool> GetRecords(JobParameter jobParameter)
        {
            try
            {
                ...
                await _b2cConsultaPlanosRepository.ExecuteTableMerge(jobParameter: jobParameter);
                await _linxMicrovixRepositoryBase.ExecuteTruncateRawTable(jobParameter);
