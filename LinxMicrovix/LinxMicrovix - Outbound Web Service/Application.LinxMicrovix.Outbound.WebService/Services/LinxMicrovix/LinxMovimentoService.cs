...
namespace Application.LinxMicrovix.Outbound.WebService.Services.LinxMicrovix
{
    public class LinxMovimentoService : ILinxMovimentoService
    {
        ...
        private static ILinxMovimentoServiceCache _linxMovimentoServiceCache { get; set; } = new LinxMovimentoServiceCache();

        public LinxMovimentoService(
            IAPICall apiCall,
            ILoggerService logger,
            ILinxMicrovixServiceBase linxMicrovixServiceBase,
            ILinxMicrovixAzureSQLRepositoryBase<LinxMovimento> linxMicrovixRepositoryBase,
        ...
        {
            IList<LinxMovimento> _listSomenteNovos = new List<LinxMovimento>();
            ...
            try
            {
                ...
                foreach (var cnpj_emp in cnpjs_emp)
                {
                    ...
                    string? response = await _apiCall.PostAsync(jobParameter: jobParameter, body: body);
                    var xmls = _linxMicrovixServiceBase.DeserializeResponseToXML(jobParameter, response, _linxMovimentoServiceCache);

                    if (xmls.Count() > 0)
                    {
                        var listRecords = DeserializeXMLToObject(jobParameter, xmls);

                        if (_linxMovimentoServiceCache.GetList().Count == 0)
                        {
                            var list_existentes = await _linxMovimentoRepository.GetRegistersExists(jobParameter: jobParameter, registros: listRecords);
                            _linxMovimentoServiceCache.AddList(list_existentes);
                        }

                        _listSomenteNovos = _linxMovimentoServiceCache.FiltrarList(listRecords);
                        if (_listSomenteNovos.Count() > 0)
                        {
                            _linxMovimentoRepository.BulkInsertIntoTableRaw(records: _listSomenteNovos, jobParameter: jobParameter);
                            for (int i = 0; i < _listSomenteNovos.Count; i++)
                            {
                                var key = _linxMovimentoServiceCache.GetKey(_listSomenteNovos[i]);
                                if (_linxMovimentoServiceCache.GetDictionaryXml().ContainsKey(key))
                                {
                                    var xml = _linxMovimentoServiceCache.GetDictionaryXml()[key];
            ...
            finally
            {
                _logger.SetLogEndDate();
                await _logger.CommitAllChanges();
                _linxMovimentoServiceCache.AddList(_listSomenteNovos);
            }
