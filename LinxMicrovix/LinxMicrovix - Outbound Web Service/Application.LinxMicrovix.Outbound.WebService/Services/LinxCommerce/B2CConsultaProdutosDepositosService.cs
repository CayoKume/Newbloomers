    ...
    public class B2CConsultaProdutosDepositosService : IB2CConsultaProdutosDepositosService
    {
        ...
        public async Task<bool> GetRecord(LinxAPIParam jobParameter, string? identificador, string? cnpj_emp)
        {
            try
            {
                ...
                var xmls = _linxMicrovixServiceBase.DeserializeResponseToXML(jobParameter, response, _b2cConsultaProdutosDepositosCache);
        ...
        {
            IList<B2CConsultaProdutosDepositos> _listSomenteNovos = new List<B2CConsultaProdutosDepositos>();
            ...
            try
            {
                ...
                foreach (var cnpj_emp in cnpjs_emp)
                {
                    ...
                    if (xmls.Count() > 0)
                    {
                        ...
                        if (_b2cConsultaProdutosDepositosCache.GetList().Count == 0)
                        {
                            ...
                            _b2cConsultaProdutosDepositosCache.AddList(list_existentes);
                        }

                        _listSomenteNovos = _b2cConsultaProdutosDepositosCache.FiltrarList(listRecords);
                        if (_listSomenteNovos.Count() > 0)
                        {
                            ...
                            {
                                var key = _b2cConsultaProdutosDepositosCache.GetKey(_listSomenteNovos[i]);
                                if (_b2cConsultaProdutosDepositosCache.GetDictionaryXml().ContainsKey(key))
                                {
                                    var xml = _b2cConsultaProdutosDepositosCache.GetDictionaryXml()[key];
            ...
            finally
            {
                ...
                _b2cConsultaProdutosDepositosCache.AddList(_listSomenteNovos);
            }
