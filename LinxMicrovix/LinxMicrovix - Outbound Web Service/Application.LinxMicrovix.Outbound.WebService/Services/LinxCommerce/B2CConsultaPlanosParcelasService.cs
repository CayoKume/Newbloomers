...
namespace Application.LinxMicrovix.Outbound.WebService.Services
{
    public class B2CConsultaPlanosParcelasService : IB2CConsultaPlanosParcelasService
    {
        ...
        private readonly ILinxMicrovixAzureSQLRepositoryBase<B2CConsultaPlanosParcelas> _linxMicrovixRepositoryBase;
        private readonly IB2CConsultaPlanosParcelasRepository _b2cConsultaPlanosParcelasRepository;
        private static IB2CConsultaPlanosParcelasServiceCache _b2cConsultaPlanosParcelasCache { get; set; } = new B2CConsultaPlanosParcelasServiceCache();

        public B2CConsultaPlanosParcelasService(
            IAPICall apiCall,
            ILoggerService logger,
            ILinxMicrovixServiceBase linxMicrovixServiceBase,
            ILinxMicrovixAzureSQLRepositoryBase<B2CConsultaPlanosParcelas> linxMicrovixRepositoryBase,
                    ...
                    if (validations.Count() > 0)
                    {
                        for (int j = 0; j < validations.Count(); j++)
                        {
                            _logger.AddMessage(
                                ...
                                message: $"Error when convert record - plano: {records[i].Where(pair => pair.Key == "plano").Select(pair => pair.Value).FirstOrDefault()}\n" +
        ...
        public async Task<bool> GetRecord(LinxAPIParam jobParameter, string? identificador, string? cnpj_emp)
        {
            try
            {
                ...
                var xmls = _linxMicrovixServiceBase.DeserializeResponseToXML(jobParameter, response, _b2cConsultaPlanosParcelasCache);
        ...
        {
            IList<B2CConsultaPlanosParcelas> _listSomenteNovos = new List<B2CConsultaPlanosParcelas>();
            ...
            try
            {
                ...
                foreach (var cnpj_emp in cnpjs_emp)
                {
                    ...
                    if (xmls.Count() > 0)
                    {
                        ...
                        if (_b2cConsultaPlanosParcelasCache.GetList().Count == 0)
                        {
                            ...
                            _b2cConsultaPlanosParcelasCache.AddList(list_existentes);
                        }

                        _listSomenteNovos = _b2cConsultaPlanosParcelasCache.FiltrarList(listRecords);
                        if (_listSomenteNovos.Count() > 0)
                        {
                            ...
                            {
                                var key = _b2cConsultaPlanosParcelasCache.GetKey(_listSomenteNovos[i]);
                                if (_b2cConsultaPlanosParcelasCache.GetDictionaryXml().ContainsKey(key))
                                {
                                    var xml = _b2cConsultaPlanosParcelasCache.GetDictionaryXml()[key];
            ...
            finally
            {
                ...
                _b2cConsultaPlanosParcelasCache.AddList(_listSomenteNovos);
            }
