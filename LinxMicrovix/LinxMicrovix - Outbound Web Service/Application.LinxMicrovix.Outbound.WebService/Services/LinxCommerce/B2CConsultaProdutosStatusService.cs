...
namespace Application.LinxMicrovix.Outbound.WebService.Services
{
    public class B2CConsultaProdutosStatusService : IB2CConsultaProdutosStatusService
    {
        ...
        private readonly ILinxMicrovixAzureSQLRepositoryBase<B2CConsultaProdutosStatus> _linxMicrovixRepositoryBase;
        private readonly IB2CConsultaProdutosStatusRepository _b2cConsultaProdutosStatusRepository;
        private static IB2CConsultaProdutosStatusServiceCache _b2cConsultaProdutosStatusCache { get; set; } = new B2CConsultaProdutosStatusServiceCache();

        public B2CConsultaProdutosStatusService(
            IAPICall apiCall,
            ILoggerService logger,
            ILinxMicrovixServiceBase linxMicrovixServiceBase,
            ILinxMicrovixAzureSQLRepositoryBase<B2CConsultaProdutosStatus> linxMicrovixRepositoryBase,
        ...
        public List<B2CConsultaProdutosStatus?> DeserializeXMLToObject(LinxAPIParam jobParameter, List<Dictionary<string?, string?>> records)
        {
            ...
            for (int i = 0; i < records.Count(); i++)
            {
                try
                {
                    ...
                    var entity = new B2CConsultaProdutosStatus(
                    ...
                    if (validations.Count() > 0)
                    {
                        for (int j = 0; j < validations.Count(); j++)
                        {
                            _logger.AddMessage(
                                ...
                                message: $"Error when convert record - codigoproduto: {records[i].Where(pair => pair.Key == "codigoproduto").Select(pair => pair.Value).FirstOrDefault()} | referencia: {records[i].Where(pair => pair.Key == "referencia").Select(pair => pair.Value).FirstOrDefault()}\n" +
        ...
        public async Task<bool> GetRecord(LinxAPIParam jobParameter, string? identificador, string? cnpj_emp)
        {
            try
            {
                ...
                var xmls = _linxMicrovixServiceBase.DeserializeResponseToXML(jobParameter, response, _b2cConsultaProdutosStatusCache);
        ...
        {
            IList<B2CConsultaProdutosStatus> _listSomenteNovos = new List<B2CConsultaProdutosStatus>();
            ...
            try
            {
                ...
                foreach (var cnpj_emp in cnpjs_emp)
                {
                    ...
                    if (xmls.Count() > 0)
                    {
                        ...
                        if (_b2cConsultaProdutosStatusCache.GetList().Count == 0)
                        {
                            ...
                            _b2cConsultaProdutosStatusCache.AddList(list_existentes);
                        }

                        _listSomenteNovos = _b2cConsultaProdutosStatusCache.FiltrarList(listRecords);
                        if (_listSomenteNovos.Count() > 0)
                        {
                            ...
                            {
                                var key = _b2cConsultaProdutosStatusCache.GetKey(_listSomenteNovos[i]);
                                if (_b2cConsultaProdutosStatusCache.GetDictionaryXml().ContainsKey(key))
                                {
                                    var xml = _b2cConsultaProdutosStatusCache.GetDictionaryXml()[key];
            ...
            finally
            {
                ...
                _b2cConsultaProdutosStatusCache.AddList(_listSomenteNovos);
            }
