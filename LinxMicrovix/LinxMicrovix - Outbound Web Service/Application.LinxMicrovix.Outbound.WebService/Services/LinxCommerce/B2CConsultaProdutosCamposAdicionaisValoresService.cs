...
namespace Application.LinxMicrovix.Outbound.WebService.Services
{
    public class B2CConsultaProdutosCamposAdicionaisValoresService : IB2CConsultaProdutosCamposAdicionaisValoresService
    {
        ...
        public B2CConsultaProdutosCamposAdicionaisValoresService(
            ...
            ILinxMicrovixAzureSQLRepositoryBase<B2CConsultaProdutosCamposAdicionaisValores> linxMicrovixRepositoryBase,
                    ...
                    if (validations.Count() > 0)
                    {
                        for (int j = 0; j < validations.Count(); j++)
                        {
                            _logger.AddMessage(
                                ...
                                message: $"Error when convert record - codigoproduto: {records[i].Where(pair => pair.Key == "codigoproduto").Select(pair => pair.Value).FirstOrDefault()} | id_campo_detalhe: {records[i].Where(pair => pair.Key == "id_campo_detalhe").Select(pair => pair.Value).FirstOrDefault()}\n" +
        ...
        {
            IList<B2CConsultaProdutosCamposAdicionaisValores> _listSomenteNovos = new List<B2CConsultaProdutosCamposAdicionaisValores>();
