...
namespace Application.LinxMicrovix.Outbound.WebService.Services
{
    public class B2CConsultaProdutosPalavrasChavePesquisaService : IB2CConsultaProdutosPalavrasChavePesquisaService
    {
        ...
        private static IB2CConsultaProdutosPalavrasChavePesquisaServiceCache _b2cConsultaProdutosPalavrasChavePesquisaCache { get; set; } = new B2CConsultaProdutosPalavrasChavePesquisaServiceCache();

        public B2CConsultaProdutosPalavrasChavePesquisaService(
            IAPICall apiCall,
            ILoggerService logger,
            ILinxMicrovixServiceBase linxMicrovixServiceBase,
            ILinxMicrovixAzureSQLRepositoryBase<B2CConsultaProdutosPalavrasChavePesquisa> linxMicrovixRepositoryBase,
                    ...
                    if (validations.Count() > 0)
                    {
                        for (int j = 0; j < validations.Count(); j++)
                        {
                            _logger.AddMessage(
                                ...
                                message: $"Error when convert record - id_b2c_palavras_chave_pesquisa: {records[i].Where(pair => pair.Key == "id_b2c_palavras_chave_pesquisa").Select(pair => pair.Value).FirstOrDefault()} | descricao_b2c_palavras_chave_pesquisa: {records[i].Where(pair => pair.Key == "descricao_b2c_palavras_chave_pesquisa").Select(pair => pair.Value).FirstOrDefault()}\n" +
        ...
        {
            IList<B2CConsultaProdutosPalavrasChavePesquisa> _listSomenteNovos = new List<B2CConsultaProdutosPalavrasChavePesquisa>();

            try
            {
                _logger
                   .Clear()
                   .AddLog(EnumJob.B2CConsultaProdutosPalavrasChavePesquisa);
                ...
                foreach (var cnpj_emp in cnpjs_emp)
                {
                    ...
                    var xmls = _linxMicrovixServiceBase.DeserializeResponseToXML(jobParameter, response, _b2cConsultaProdutosPalavrasChavePesquisaCache);

                    if (xmls.Count() > 0)
                    {
                        var listRecords = DeserializeXMLToObject(jobParameter, xmls);

                        if (_b2cConsultaProdutosPalavrasChavePesquisaCache.GetList().Count == 0)
                        {
                            var list_existentes = await _b2cConsultaProdutosPalavrasChavePesquisaRepository.GetRegistersExists(jobParameter: jobParameter, registros: listRecords);
                            _b2cConsultaProdutosPalavrasChavePesquisaCache.AddList(list_existentes);
                        }

                        _listSomenteNovos = _b2cConsultaProdutosPalavrasChavePesquisaCache.FiltrarList(listRecords);
                        if (_listSomenteNovos.Count() > 0)
                        {
                            _b2cConsultaProdutosPalavrasChavePesquisaRepository.BulkInsertIntoTableRaw(records: _listSomenteNovos, jobParameter: jobParameter);
                            for (int i = 0; i < _listSomenteNovos.Count; i++)
                            {
                                var key = _b2cConsultaProdutosPalavrasChavePesquisaCache.GetKey(_listSomenteNovos[i]);
                                if (_b2cConsultaProdutosPalavrasChavePesquisaCache.GetDictionaryXml().ContainsKey(key))
                                {
                                    var xml = _b2cConsultaProdutosPalavrasChavePesquisaCache.GetDictionaryXml()[key];
            ...
            finally
            {
                ...
                _b2cConsultaProdutosPalavrasChavePesquisaCache.AddList(_listSomenteNovos);
            }
