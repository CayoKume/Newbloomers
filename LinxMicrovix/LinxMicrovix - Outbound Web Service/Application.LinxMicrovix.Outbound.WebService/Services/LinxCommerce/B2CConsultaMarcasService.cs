...
namespace Application.LinxMicrovix.Outbound.WebService.Services
{
    public class B2CConsultaMarcasService : IB2CConsultaMarcasService
    {
        ...
        private static IB2CConsultaMarcasServiceCache _b2cConsultaMarcasCache { get; set; } = new B2CConsultaMarcasServiceCache();

        public B2CConsultaMarcasService(
            IAPICall apiCall,
            ILoggerService logger,
            ILinxMicrovixServiceBase linxMicrovixServiceBase,
            ILinxMicrovixAzureSQLRepositoryBase<B2CConsultaMarcas> linxMicrovixRepositoryBase,
                    ...
                    if (validations.Count() > 0)
                    {
                        for (int j = 0; j < validations.Count(); j++)
                        {
                            _logger.AddMessage(
                                ...
                                message: $"Error when convert record - codigo_marca: {records[i].Where(pair => pair.Key == "codigo_marca").Select(pair => pair.Value).FirstOrDefault()} | nome_marca: {records[i].Where(pair => pair.Key == "nome_marca").Select(pair => pair.Value).FirstOrDefault()}\n" +
        ...
        public async Task<bool> GetRecord(LinxAPIParam jobParameter, string? identificador, string? cnpj_emp)
        {
            try
            {
                ...
                var xmls = _linxMicrovixServiceBase.DeserializeResponseToXML(jobParameter, response, _b2cConsultaMarcasCache);
        ...
        {
            IList<B2CConsultaMarcas> _listSomenteNovos = new List<B2CConsultaMarcas>();
            ...
            try
            {
                ...
                foreach (var cnpj_emp in cnpjs_emp)
                {
                    ...
                    if (xmls.Count() > 0)
                    {
                        ...
                        if (_b2cConsultaMarcasCache.GetList().Count == 0)
                        {
                            ...
                            _b2cConsultaMarcasCache.AddList(list_existentes);
                        }

                        _listSomenteNovos = _b2cConsultaMarcasCache.FiltrarList(listRecords);
                        if (_listSomenteNovos.Count() > 0)
                        {
                            _b2cConsultaMarcasRepository.BulkInsertIntoTableRaw(records: _listSomenteNovos, jobParameter: jobParameter);
                            for (int i = 0; i < _listSomenteNovos.Count; i++)
                            {
                                var key = _b2cConsultaMarcasCache.GetKey(_listSomenteNovos[i]);
                                if (_b2cConsultaMarcasCache.GetDictionaryXml().ContainsKey(key))
                                {
                                    var xml = _b2cConsultaMarcasCache.GetDictionaryXml()[key];
            ...
            finally
            {
                ...
                _b2cConsultaMarcasCache.AddList(_listSomenteNovos);
            }
