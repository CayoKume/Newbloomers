...
namespace Application.LinxMicrovix.Outbound.WebService.Services
{
    public class B2CConsultaEspessurasService : IB2CConsultaEspessurasService
    {
        ...
        private static IB2CConsultaEspessurasServiceCache _b2cConsultaEspessurasCache = new B2CConsultaEspessurasServiceCache();

        public B2CConsultaEspessurasService(
            IAPICall apiCall,
            ILoggerService logger,
            ILinxMicrovixServiceBase linxMicrovixServiceBase,
            ILinxMicrovixAzureSQLRepositoryBase<B2CConsultaEspessuras> linxMicrovixRepositoryBase,
        ...
        public async Task<bool> GetRecord(LinxAPIParam jobParameter, string? identificador, string? cnpj_emp)
        {
            try
            {
                ...
                var xmls = _linxMicrovixServiceBase.DeserializeResponseToXML(jobParameter, response, _b2cConsultaEspessurasCache);
        ...
        {
            IList<B2CConsultaEspessuras> _listSomenteNovos = new List<B2CConsultaEspessuras>();
            ...
            try
            {
                ...
                foreach (var cnpj_emp in cnpjs_emp)
                {
                    ...
                    if (xmls.Count() > 0)
                    {
                        ...
                        if (_b2cConsultaEspessurasCache.GetList().Count == 0)
                        {
                            ...
                            _b2cConsultaEspessurasCache.AddList(list_existentes);
                        }

                        _listSomenteNovos = _b2cConsultaEspessurasCache.FiltrarList(listRecords);
                        if (_listSomenteNovos.Count() > 0)
                        {
                            _b2cConsultaEspessurasRepository.BulkInsertIntoTableRaw(records: _listSomenteNovos, jobParameter: jobParameter);
                            for (int i = 0; i < _listSomenteNovos.Count; i++)
                            {
                                var key = _b2cConsultaEspessurasCache.GetKey(_listSomenteNovos[i]);
                                if (_b2cConsultaEspessurasCache.GetDictionaryXml().ContainsKey(key))
                                {
                                    var xml = _b2cConsultaEspessurasCache.GetDictionaryXml()[key];
            ...
            finally
            {
                ...
                _b2cConsultaEspessurasCache.AddList(_listSomenteNovos);
            }
