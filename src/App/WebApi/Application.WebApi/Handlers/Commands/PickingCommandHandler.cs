...
{
    public class PickingCommandHandler : IPickingCommandHandler
    {
        public string CreateGetShippingCompanysQuery()
        {
            return $@"SELECT 
        ...
        public string CreateGetUnpickedOrderQuery(string cnpj_emp, string serie, string nr_pedido)
        {
        ...
        public string CreateGetUnpickedOrdersQuery(string cnpj_emp, string serie_pedido, string data_inicial, string data_final)
        {
            return $@"SELECT DISTINCT
                        TRIM(A.DOCUMENTO) AS NUMBER,
                        A.RETORNO AS RETORNO,
        ...
        public string CreateGetUnpickedOrdersToPrintQuery(string cnpj_emp, string serie_pedido, string data_inicial, string data_final)
        {
            throw new NotImplementedException();
        }

        public string CreateGetUnpickedOrderToPrintQuery(string cnpj_emp, string serie, string nr_pedido)
        {
            ...
            {
                return sql.Replace("[1]", "OBS").Replace("[0]", $@"FROM GENERAL..IT4_WMS_DOCUMENTO A (NOLOCK)
                                      JOIN GENERAL..IT4_WMS_DOCUMENTO_ITEM B (NOLOCK) ON A.IDCONTROLE = B.IDCONTROLE
            ...
            return sql.Replace("[1]", "ANOTACAO").Replace("[0]", $@"FROM GENERAL..IT4_WMS_DOCUMENTO A (NOLOCK)
                                    JOIN GENERAL..IT4_WMS_DOCUMENTO_ITEM B (NOLOCK) ON A.IDCONTROLE = B.IDCONTROLE
        ...
        public string CreateUpdateRetornoQuery(string nr_pedido, int volumes, string listProdutos)
        {
            var listaDeProdutos = JsonConvert.DeserializeObject<List<PickingProduct>>(listProdutos);
        ...
        public string CreateUpdateShippingCompanyQuery(string nr_pedido, int cod_transportador)
        {
            return $@"UPDATE A SET
