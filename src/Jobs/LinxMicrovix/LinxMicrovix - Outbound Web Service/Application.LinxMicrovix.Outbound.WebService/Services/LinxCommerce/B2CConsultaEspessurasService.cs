...
namespace Application.LinxMicrovix.Outbound.WebService.Services
{
    public class B2CConsultaEspessurasService : IB2CConsultaEspessurasService
    {
        ...
        private readonly IValidator<Domain.LinxMicrovix.Outbound.WebService.Dtos.LinxCommerce.B2CConsultaEspessuras> _validator;
        ...
        public B2CConsultaEspessurasService(
            ...
            IValidator<Domain.LinxMicrovix.Outbound.WebService.Dtos.LinxCommerce.B2CConsultaEspessuras> validator,
        ...
        public List<B2CConsultaEspessuras?> DeserializeXMLToObject(LinxAPIParam jobParameter, List<Dictionary<string?, string?>> records)
        {
            ...
            for (int i = 0; i < records.Count(); i++)
            {
                try
                {
                    ...
                    {
                        var message = $"Error when convert record - codigo_espessura: {records[i].Where(pair => pair.Key == ";
                    ...
                    list.Add(new B2CConsultaEspessuras(entity, xml));
                }
                catch (Exception ex)
                {
                    throw new GeneralException(
                        message: $"Error when convert record - codigo_espessura: {records[i].Where(pair => pair.Key == " - {ex.Message}codigo_espessura").Select(pair => pair.Value).FirstOrDefault()} | nome_espessura: {records[i].Where(pair => pair.Key == "nome_espessura").Select(pair => pair.Value).FirstOrDefault()} - {ex.Message}",
