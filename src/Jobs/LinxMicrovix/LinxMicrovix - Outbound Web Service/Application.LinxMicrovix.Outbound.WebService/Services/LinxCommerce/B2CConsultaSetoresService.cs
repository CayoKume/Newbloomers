        ...
        public List<B2CConsultaSetores?> DeserializeXMLToObject(LinxAPIParam jobParameter, List<Dictionary<string?, string?>> records)
        {
            ...
            for (int i = 0; i < records.Count(); i++)
            {
                try
                {
                    ...
                    {
                        var message = $"Error when convert record - codigo_setor: {records[i].Where(pair => pair.Key == ";
                ...
                catch (Exception ex)
                {
                    throw new GeneralException(
                        message: $"Error when convert record - codigo_setor: {records[i].Where(pair => pair.Key == " - {ex.Message}codigo_setor").Select(pair => pair.Value).FirstOrDefault()} | nome_setor: {records[i].Where(pair => pair.Key == "nome_setor").Select(pair => pair.Value).FirstOrDefault()} - {ex.Message}",
