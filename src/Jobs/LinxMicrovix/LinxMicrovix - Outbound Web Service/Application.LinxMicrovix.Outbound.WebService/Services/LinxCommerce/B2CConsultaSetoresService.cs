...
namespace Application.LinxMicrovix.Outbound.WebService.Services
{
    public class B2CConsultaSetoresService : IB2CConsultaSetoresService
    {
        ...
        private readonly IValidator<Domain.LinxMicrovix.Outbound.WebService.Dtos.LinxCommerce.B2CConsultaSetores> _validator;
        private static List<string?> _b2cConsultaSetoresCache { get; set; } = new List<string?>();
        ...
        public B2CConsultaSetoresService(
            ...
            IValidator<Domain.LinxMicrovix.Outbound.WebService.Dtos.LinxCommerce.B2CConsultaSetores> validator,
        ...
        public List<B2CConsultaSetores?> DeserializeXMLToObject(LinxAPIParam jobParameter, List<Dictionary<string?, string?>> records)
        {
            ...
            for (int i = 0; i < records.Count(); i++)
            {
                try
                {
                    var entity = new Domain.LinxMicrovix.Outbound.WebService.Dtos.LinxCommerce.B2CConsultaSetores(
                    ...
                    {
                        var message = $"Error when convert record - codigo_setor: {records[i].Where(pair => pair.Key == "codigo_setor").Select(pair => pair.Value).FirstOrDefault()} | nome_setor: {records[i].Where(pair => pair.Key == "nome_setor").Select(pair => pair.Value).FirstOrDefault()} ";
                    ...
                    list.Add(new B2CConsultaSetores(entity, xml));
                }
                catch (Exception ex)
                {
                    throw new GeneralException(
                        message: $"Error when convert record - codigo_setor: {records[i].Where(pair => pair.Key == "codigo_setor").Select(pair => pair.Value).FirstOrDefault()} | nome_setor: {records[i].Where(pair => pair.Key == "nome_setor").Select(pair => pair.Value).FirstOrDefault()} - {ex.Message}",
        ...
        public async Task<bool> GetRecord(LinxAPIParam jobParameter, string? identificador, string? cnpj_emp)
        {
            _logger
               .Clear()
               .AddLog(EnumJob.B2CConsultaSetores);
        ...
        public async Task<bool> GetRecords(LinxAPIParam jobParameter)
        {
            ...
            foreach (var cnpj_emp in cnpjs_emp)
            {
                ...
                if (xmls.Count() > 0)
                {
                    ...
                    if (_b2cConsultaSetoresCache.Count == 0)
                    {
                        var list_existentes = await _b2cConsultaSetoresRepository.GetRegistersExists(jobParameter: jobParameter, registros: listRecords);
                        _b2cConsultaSetoresCache = list_existentes.ToList();
                    }

                    var _listSomenteNovos = listRecords.Where(x => !_b2cConsultaSetoresCache.Any(y =>
                        y == x.recordKey
                    )).ToList();

                    if (_listSomenteNovos.Count() > 0)
                    {
                        _b2cConsultaSetoresRepository.BulkInsertIntoTableRaw(records: _listSomenteNovos, jobParameter: jobParameter);
                        await _coreRepository.CallDbProcMerge(jobParameter.schema, jobParameter.tableName, _logger.GetExecutionGuid());
                        ...
                        _b2cConsultaSetoresCache.AddRange(_listSomenteNovos.Select(x => x.recordKey));
                    }
