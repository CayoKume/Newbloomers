...
namespace Application.LinxMicrovix.Outbound.WebService.Services
{
    public class B2CConsultaProdutosDepositosService : IB2CConsultaProdutosDepositosService
    {
        ...
        private readonly IValidator<Domain.LinxMicrovix.Outbound.WebService.Dtos.LinxCommerce.B2CConsultaProdutosDepositos> _validator;
        ...
        public B2CConsultaProdutosDepositosService(
            ...
            IValidator<Domain.LinxMicrovix.Outbound.WebService.Dtos.LinxCommerce.B2CConsultaProdutosDepositos> validator,
        ...
        public List<B2CConsultaProdutosDepositos?> DeserializeXMLToObject(LinxAPIParam jobParameter, List<Dictionary<string?, string?>> records)
        {
            ...
            for (int i = 0; i < records.Count(); i++)
            {
                try
                {
                    ...
                    var entity = new Domain.LinxMicrovix.Outbound.WebService.Dtos.LinxCommerce.B2CConsultaProdutosDepositos(
                    ...
                    {
                        var message = $"Error when convert record - id_deposito: {records[i].Where(pair => pair.Key == ";
                    ...
                    list.Add(new B2CConsultaProdutosDepositos(entity, xml));
                }
                catch (Exception ex)
                {
                    throw new GeneralException(
                        message: $"Error when convert record - id_deposito: {records[i].Where(pair => pair.Key == " - {ex.Message}id_deposito").Select(pair => pair.Value).FirstOrDefault()} | nome_deposito: {records[i].Where(pair => pair.Key == "nome_deposito").Select(pair => pair.Value).FirstOrDefault()} - {ex.Message}",
