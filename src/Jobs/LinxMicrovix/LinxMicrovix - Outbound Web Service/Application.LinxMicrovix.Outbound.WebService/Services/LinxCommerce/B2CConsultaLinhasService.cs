...
namespace Application.LinxMicrovix.Outbound.WebService.Services
{
    public class B2CConsultaLinhasService : IB2CConsultaLinhasService
    {
        ...
        private readonly IValidator<Domain.LinxMicrovix.Outbound.WebService.Dtos.LinxCommerce.B2CConsultaLinhas> _validator;
        private static List<string?> _b2cConsultaLinhasCache { get; set; } = new List<string?>();
        ...
        public B2CConsultaLinhasService(
            ...
            IValidator<Domain.LinxMicrovix.Outbound.WebService.Dtos.LinxCommerce.B2CConsultaLinhas> validator,
        ...
        public List<B2CConsultaLinhas?> DeserializeXMLToObject(LinxAPIParam jobParameter, List<Dictionary<string?, string?>> records)
        {
            ...
            for (int i = 0; i < records.Count(); i++)
            {
                try
                {
                    var entity = new Domain.LinxMicrovix.Outbound.WebService.Dtos.LinxCommerce.B2CConsultaLinhas(
                    ...
                    {
                        var message = $"Error when convert record - codigo_linha: {records[i].Where(pair => pair.Key == "codigo_linha").Select(pair => pair.Value).FirstOrDefault()} | nome_linha: {records[i].Where(pair => pair.Key == "nome_linha").Select(pair => pair.Value).FirstOrDefault()} ";
                    ...
                    list.Add(new B2CConsultaLinhas(entity, xml));
                }
                catch (Exception ex)
                {
                    throw new GeneralException(
                        message: $"Error when convert record - codigo_linha: {records[i].Where(pair => pair.Key == "codigo_linha").Select(pair => pair.Value).FirstOrDefault()} | nome_linha: {records[i].Where(pair => pair.Key == "nome_linha").Select(pair => pair.Value).FirstOrDefault()} - {ex.Message}",
        ...
        public async Task<bool> GetRecord(LinxAPIParam jobParameter, string? identificador, string? cnpj_emp)
        {
            _logger
               .Clear()
               .AddLog(EnumJob.B2CConsultaLinhas);
        ...
        public async Task<bool> GetRecords(LinxAPIParam jobParameter)
        {
            ...
            foreach (var cnpj_emp in cnpjs_emp)
            {
                ...
                if (xmls.Count() > 0)
                {
                    ...
                    if (_b2cConsultaLinhasCache.Count == 0)
                    {
                        var list_existentes = await _b2cConsultaLinhasRepository.GetRegistersExists(jobParameter: jobParameter, registros: listRecords);
                        _b2cConsultaLinhasCache = list_existentes.ToList();
                    }

                    var _listSomenteNovos = listRecords.Where(x => !_b2cConsultaLinhasCache.Any(y =>
                        y == x.recordKey
                    )).ToList();

                    if (_listSomenteNovos.Count() > 0)
                    {
                        _b2cConsultaLinhasRepository.BulkInsertIntoTableRaw(records: _listSomenteNovos, jobParameter: jobParameter);
                        await _coreRepository.CallDbProcMerge(jobParameter.schema, jobParameter.tableName, _logger.GetExecutionGuid());
                        ...
                        _b2cConsultaLinhasCache.AddRange(_listSomenteNovos.Select(x => x.recordKey));
                    }
