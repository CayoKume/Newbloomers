...
namespace Application.LinxMicrovix.Outbound.WebService.Services
{
    public class B2CConsultaEmpresasService : IB2CConsultaEmpresasService
    {
        ...
        private readonly IValidator<Domain.LinxMicrovix.Outbound.WebService.Dtos.LinxCommerce.B2CConsultaEmpresas> _validator;
        ...
        public B2CConsultaEmpresasService(
            ...
            IValidator<Domain.LinxMicrovix.Outbound.WebService.Dtos.LinxCommerce.B2CConsultaEmpresas> validator,
        ...
        public List<B2CConsultaEmpresas?> DeserializeXMLToObject(LinxAPIParam jobParameter, List<Dictionary<string?, string?>> records)
        {
            ...
            for (int i = 0; i < records.Count(); i++)
            {
                try
                {
                    ...
                    var entity = new Domain.LinxMicrovix.Outbound.WebService.Dtos.LinxCommerce.B2CConsultaEmpresas(
                    ...
                    {
                        var message = $"Error when convert record - empresa: {records[i].Where(pair => pair.Key == ";
                    ...
                    list.Add(new B2CConsultaEmpresas(entity, xml));
                }
                catch (Exception ex)
                {
                    throw new GeneralException(
                        message: $"Error when convert record - empresa: {records[i].Where(pair => pair.Key == " - {ex.Message}empresa").Select(pair => pair.Value).FirstOrDefault()} | nome_emp: {records[i].Where(pair => pair.Key == "nome_emp").Select(pair => pair.Value).FirstOrDefault()} - {ex.Message}",
