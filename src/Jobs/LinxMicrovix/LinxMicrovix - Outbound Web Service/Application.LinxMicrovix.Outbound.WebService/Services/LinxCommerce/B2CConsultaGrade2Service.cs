...
namespace Application.LinxMicrovix.Outbound.WebService.Services
{
    public class B2CConsultaGrade2Service : IB2CConsultaGrade2Service
    {
        ...
        private readonly IValidator<Domain.LinxMicrovix.Outbound.WebService.Dtos.LinxCommerce.B2CConsultaGrade2> _validator;
        private static List<string?> _b2cConsultaGrade2Cache { get; set; } = new List<string?>();
        ...
        public B2CConsultaGrade2Service(
            ...
            IValidator<Domain.LinxMicrovix.Outbound.WebService.Dtos.LinxCommerce.B2CConsultaGrade2> validator,
        ...
        public List<B2CConsultaGrade2?> DeserializeXMLToObject(LinxAPIParam jobParameter, List<Dictionary<string?, string?>> records)
        {
            ...
            for (int i = 0; i < records.Count(); i++)
            {
                try
                {
                    var entity = new Domain.LinxMicrovix.Outbound.WebService.Dtos.LinxCommerce.B2CConsultaGrade2(
                    ...
                    {
                        var message = $"Error when convert record - codigo_grade2: {records[i].Where(pair => pair.Key == "codigo_grade2").Select(pair => pair.Value).FirstOrDefault()} | nome_grade2: {records[i].Where(pair => pair.Key == "nome_grade2").Select(pair => pair.Value).FirstOrDefault()} ";
                    ...
                    list.Add(new B2CConsultaGrade2(entity, xml));
                }
                catch (Exception ex)
                {
                    throw new GeneralException(
                        message: $"Error when convert record - codigo_grade2: {records[i].Where(pair => pair.Key == "codigo_grade2").Select(pair => pair.Value).FirstOrDefault()} | nome_grade2: {records[i].Where(pair => pair.Key == "nome_grade2").Select(pair => pair.Value).FirstOrDefault()} - {ex.Message}",
        ...
        public async Task<bool> GetRecords(LinxAPIParam jobParameter)
        {
            ...
            foreach (var cnpj_emp in cnpjs_emp)
            {
                ...
                if (xmls.Count() > 0)
                {
                    ...
                    if (_b2cConsultaGrade2Cache.Count == 0)
                    {
                        ...
                        _b2cConsultaGrade2Cache = list_existentes.ToList();
                    }

                    var _listSomenteNovos = listRecords.Where(x => !_b2cConsultaGrade2Cache.Any(y =>
                        y == x.recordKey
                    )).ToList();
                    ...
                    if (_listSomenteNovos.Count() > 0)
                    {
                        ...
                        _b2cConsultaGrade2Cache.AddRange(_listSomenteNovos.Select(x => x.recordKey));
                    }
