...
namespace Application.LinxMicrovix.Outbound.WebService.Services
{
    public class B2CConsultaColecoesService : IB2CConsultaColecoesService
    {
        ...
        private readonly IValidator<Domain.LinxMicrovix.Outbound.WebService.Dtos.LinxCommerce.B2CConsultaColecoes> _validator;
        ...
        public B2CConsultaColecoesService(
            ...
            IValidator<Domain.LinxMicrovix.Outbound.WebService.Dtos.LinxCommerce.B2CConsultaColecoes> validator,
        ...
        public List<B2CConsultaColecoes?> DeserializeXMLToObject(LinxAPIParam jobParameter, List<Dictionary<string?, string?>> records)
        {
            ...
            for (int i = 0; i < records.Count(); i++)
            {
                try
                {
                    ...
                    var entity = new Domain.LinxMicrovix.Outbound.WebService.Dtos.LinxCommerce.B2CConsultaColecoes(
                    ...
                    {
                        var message = $"Error when convert record - codigo_colecao: {records[i].Where(pair => pair.Key == ";
                    ...
                    list.Add(new B2CConsultaColecoes(entity, xml));
                }
                catch (Exception ex)
                {
                    throw new GeneralException(
                        message: $"Error when convert record - codigo_colecao: {records[i].Where(pair => pair.Key == " - {ex.Message}codigo_colecao").Select(pair => pair.Value).FirstOrDefault()} | nome_colecao: {records[i].Where(pair => pair.Key == "nome_colecao").Select(pair => pair.Value).FirstOrDefault()} - {ex.Message}",
