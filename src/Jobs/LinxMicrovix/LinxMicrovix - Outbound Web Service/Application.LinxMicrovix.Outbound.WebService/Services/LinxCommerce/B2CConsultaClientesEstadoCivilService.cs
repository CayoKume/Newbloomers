        ...
        public List<B2CConsultaClientesEstadoCivil?> DeserializeXMLToObject(LinxAPIParam jobParameter, List<Dictionary<string?, string?>> records)
        {
            ...
            for (int i = 0; i < records.Count(); i++)
            {
                try
                {
                    ...
                    {
                        var message = $"Error when convert record - id_estado_civil: {records[i].Where(pair => pair.Key == ";
                ...
                catch (Exception ex)
                {
                    throw new GeneralException(
                        message: $"Error when convert record - id_estado_civil: {records[i].Where(pair => pair.Key == " - {ex.Message}id_estado_civil").Select(pair => pair.Value).FirstOrDefault()} | estado_civil:{records[i].Where(pair => pair.Key == "estado_civil").Select(pair => pair.Value).FirstOrDefault()} - {ex.Message}",
