        ...
        public List<B2CConsultaClientesSaldo?> DeserializeXMLToObject(LinxAPIParam jobParameter, List<Dictionary<string?, string?>> records)
        {
            ...
            for (int i = 0; i < records.Count(); i++)
            {
                try
                {
                    ...
                    {
                        var message = $"Error when convert record - cod_cliente_erp: {records[i].Where(pair => pair.Key == ";
                ...
                catch (Exception ex)
                {
                    throw new GeneralException(
                        message: $"Error when convert record - cod_cliente_erp: {records[i].Where(pair => pair.Key == " - {ex.Message}cod_cliente_erp").Select(pair => pair.Value).FirstOrDefault()} | saldo: {records[i].Where(pair => pair.Key == "saldo").Select(pair => pair.Value).FirstOrDefault()} - {ex.Message}",
