...
namespace Application.LinxMicrovix.Outbound.WebService.Services
{
    public class B2CConsultaProdutosCustosService : IB2CConsultaProdutosCustosService
    {
        ...
        private readonly IValidator<Domain.LinxMicrovix.Outbound.WebService.Dtos.LinxCommerce.B2CConsultaProdutosCustos> _validator;
        ...
        public B2CConsultaProdutosCustosService(
            ...
            IValidator<Domain.LinxMicrovix.Outbound.WebService.Dtos.LinxCommerce.B2CConsultaProdutosCustos> validator,
        ...
        public List<B2CConsultaProdutosCustos?> DeserializeXMLToObject(LinxAPIParam jobParameter, List<Dictionary<string?, string?>> records)
        {
            ...
            for (int i = 0; i < records.Count(); i++)
            {
                try
                {
                    ...
                    var entity = new Domain.LinxMicrovix.Outbound.WebService.Dtos.LinxCommerce.B2CConsultaProdutosCustos(
                    ...
                    {
                        var message = $"Error when convert record - id_produtos_custos: {records[i].Where(pair => pair.Key == ";
                    ...
                    list.Add(new B2CConsultaProdutosCustos(entity, xml));
                }
                catch (Exception ex)
                {
                    throw new GeneralException(
                        message: $"Error when convert record - id_produtos_custos: {records[i].Where(pair => pair.Key == " - {ex.Message}id_produtos_custos").Select(pair => pair.Value).FirstOrDefault()} | codigoproduto: {records[i].Where(pair => pair.Key == "codigoproduto").Select(pair => pair.Value).FirstOrDefault()} - {ex.Message}",
