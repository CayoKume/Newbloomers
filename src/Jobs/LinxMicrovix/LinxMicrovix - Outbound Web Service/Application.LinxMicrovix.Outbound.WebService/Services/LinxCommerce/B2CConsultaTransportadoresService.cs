...
namespace Application.LinxMicrovix.Outbound.WebService.Services
{
    public class B2CConsultaTransportadoresService : IB2CConsultaTransportadoresService
    {
        ...
        private readonly IValidator<Domain.LinxMicrovix.Outbound.WebService.Dtos.LinxCommerce.B2CConsultaTransportadores> _validator;
        ...
        public B2CConsultaTransportadoresService(
            ...
            IValidator<Domain.LinxMicrovix.Outbound.WebService.Dtos.LinxCommerce.B2CConsultaTransportadores> validator,
        ...
        public List<B2CConsultaTransportadores?> DeserializeXMLToObject(LinxAPIParam jobParameter, List<Dictionary<string?, string?>> records)
        {
            ...
            for (int i = 0; i < records.Count(); i++)
            {
                try
                {
                    ...
                    var entity = new Domain.LinxMicrovix.Outbound.WebService.Dtos.LinxCommerce.B2CConsultaTransportadores(
                    ...
                    {
                        var message = $"Error when convert record - cod_transportador: {records[i].Where(pair => pair.Key == ";
                    ...
                    list.Add(new B2CConsultaTransportadores(entity, xml));
                }
                catch (Exception ex)
                {
                    throw new GeneralException(
                        message: $"Error when convert record - cod_transportador: {records[i].Where(pair => pair.Key == " - {ex.Message}cod_transportador").Select(pair => pair.Value).FirstOrDefault()} | nome_fantasia: {records[i].Where(pair => pair.Key == "nome_fantasia").Select(pair => pair.Value).FirstOrDefault()} - {ex.Message}",
