...
namespace Application.LinxMicrovix.Outbound.WebService.Services
{
    public class B2CConsultaMarcasService : IB2CConsultaMarcasService
    {
        ...
        private readonly IValidator<Domain.LinxMicrovix.Outbound.WebService.Dtos.LinxCommerce.B2CConsultaMarcas> _validator;
        private static List<string?> _b2cConsultaMarcasCache { get; set; } = new List<string?>();
        ...
        public B2CConsultaMarcasService(
            ...
            IValidator<Domain.LinxMicrovix.Outbound.WebService.Dtos.LinxCommerce.B2CConsultaMarcas> validator,
        ...
        public List<B2CConsultaMarcas?> DeserializeXMLToObject(LinxAPIParam jobParameter, List<Dictionary<string?, string?>> records)
        {
            ...
            for (int i = 0; i < records.Count(); i++)
            {
                try
                {
                    ...
                    {
                        var message = $"Error when convert record - codigo_marca: {records[i].Where(pair => pair.Key == "codigo_marca").Select(pair => pair.Value).FirstOrDefault()} | nome_marca: {records[i].Where(pair => pair.Key == "nome_marca").Select(pair => pair.Value).FirstOrDefault()} ";
                    ...
                    list.Add(new B2CConsultaMarcas(entity, xml));
                }
                catch (Exception ex)
                {
                    throw new GeneralException(
                        message: $"Error when convert record - codigo_marca: {records[i].Where(pair => pair.Key == "codigo_marca").Select(pair => pair.Value).FirstOrDefault()} | nome_marca: {records[i].Where(pair => pair.Key == "nome_marca").Select(pair => pair.Value).FirstOrDefault()} - {ex.Message}",
        ...
        public async Task<bool> GetRecord(LinxAPIParam jobParameter, string? identificador, string? cnpj_emp)
        {
            _logger
               .Clear()
               .AddLog(EnumJob.B2CConsultaMarcas);
        ...
        public async Task<bool> GetRecords(LinxAPIParam jobParameter)
        {
            ...
            foreach (var cnpj_emp in cnpjs_emp)
            {
                ...
                if (xmls.Count() > 0)
                {
                    ...
                    if (_b2cConsultaMarcasCache.Count == 0)
                    {
                        var list_existentes = await _b2cConsultaMarcasRepository.GetRegistersExists(jobParameter: jobParameter, registros: listRecords);
                        _b2cConsultaMarcasCache = list_existentes.ToList();
                    }

                    var _listSomenteNovos = listRecords.Where(x => !_b2cConsultaMarcasCache.Any(y =>
                        y == x.recordKey
                    )).ToList();

                    if (_listSomenteNovos.Count() > 0)
                    {
                        _b2cConsultaMarcasRepository.BulkInsertIntoTableRaw(records: _listSomenteNovos, jobParameter: jobParameter);
                        await _coreRepository.CallDbProcMerge(jobParameter.schema, jobParameter.tableName, _logger.GetExecutionGuid());
                        ...
                        _b2cConsultaMarcasCache.AddRange(_listSomenteNovos.Select(x => x.recordKey));
                    }
