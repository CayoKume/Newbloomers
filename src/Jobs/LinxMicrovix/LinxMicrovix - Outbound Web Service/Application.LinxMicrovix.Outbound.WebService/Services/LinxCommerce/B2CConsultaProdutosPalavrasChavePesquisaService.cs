...
namespace Application.LinxMicrovix.Outbound.WebService.Services
{
    public class B2CConsultaProdutosPalavrasChavePesquisaService : IB2CConsultaProdutosPalavrasChavePesquisaService
    {
        ...
        private readonly IValidator<Domain.LinxMicrovix.Outbound.WebService.Dtos.LinxCommerce.B2CConsultaProdutosPalavrasChavePesquisa> _validator;
        private static List<string?> _b2cConsultaProdutosPalavrasChavePesquisaCache { get; set; } = new List<string?>();
        ...
        public B2CConsultaProdutosPalavrasChavePesquisaService(
            ...
            IValidator<Domain.LinxMicrovix.Outbound.WebService.Dtos.LinxCommerce.B2CConsultaProdutosPalavrasChavePesquisa> validator,
        ...
        public List<B2CConsultaProdutosPalavrasChavePesquisa?> DeserializeXMLToObject(LinxAPIParam jobParameter, List<Dictionary<string?, string?>> records)
        {
            ...
            for (int i = 0; i < records.Count(); i++)
            {
                try
                {
                    var entity = new Domain.LinxMicrovix.Outbound.WebService.Dtos.LinxCommerce.B2CConsultaProdutosPalavrasChavePesquisa(
                    ...
                    {
                        var message = $"Error when convert record - id_b2c_palavras_chave_pesquisa: {records[i].Where(pair => pair.Key == "id_b2c_palavras_chave_pesquisa").Select(pair => pair.Value).FirstOrDefault()} | descricao_b2c_palavras_chave_pesquisa: {records[i].Where(pair => pair.Key == "descricao_b2c_palavras_chave_pesquisa").Select(pair => pair.Value).FirstOrDefault()} ";
                    ...
                    list.Add(new B2CConsultaProdutosPalavrasChavePesquisa(entity, xml));
                }
                catch (Exception ex)
                {
        ...
        public async Task<bool> GetRecords(LinxAPIParam jobParameter)
        {
            ...
            foreach (var cnpj_emp in cnpjs_emp)
            {
                ...
                if (xmls.Count() > 0)
                {
                    ...
                    if (_b2cConsultaProdutosPalavrasChavePesquisaCache.Count == 0)
                    {
                        ...
                        _b2cConsultaProdutosPalavrasChavePesquisaCache = list_existentes.ToList();
                    }

                    var _listSomenteNovos = listRecords.Where(x => !_b2cConsultaProdutosPalavrasChavePesquisaCache.Any(y =>
                        y == x.recordKey
                    )).ToList();
                    ...
                    if (_listSomenteNovos.Count() > 0)
                    {
                        ...
                        _b2cConsultaProdutosPalavrasChavePesquisaCache.AddRange(_listSomenteNovos.Select(x => x.recordKey));
                    }
