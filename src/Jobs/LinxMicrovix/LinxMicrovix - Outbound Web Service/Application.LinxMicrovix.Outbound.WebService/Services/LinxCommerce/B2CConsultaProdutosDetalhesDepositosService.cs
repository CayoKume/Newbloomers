...
namespace Application.LinxMicrovix.Outbound.WebService.Services
{
    public class B2CConsultaProdutosDetalhesDepositosService : IB2CConsultaProdutosDetalhesDepositosService
    {
        ...
        private readonly IValidator<Domain.LinxMicrovix.Outbound.WebService.Dtos.LinxCommerce.B2CConsultaProdutosDetalhesDepositos> _validator;
        private static List<string?> _b2cConsultaProdutosDetalhesDepositosCache { get; set; } = new List<string?>();
        ...
        public B2CConsultaProdutosDetalhesDepositosService(
            ...
            IValidator<Domain.LinxMicrovix.Outbound.WebService.Dtos.LinxCommerce.B2CConsultaProdutosDetalhesDepositos> validator,
        ...
        public List<B2CConsultaProdutosDetalhesDepositos?> DeserializeXMLToObject(LinxAPIParam jobParameter, List<Dictionary<string?, string?>> records)
        {
            ...
            for (int i = 0; i < records.Count(); i++)
            {
                try
                {
                    var entity = new Domain.LinxMicrovix.Outbound.WebService.Dtos.LinxCommerce.B2CConsultaProdutosDetalhesDepositos(
                    ...
                    {
                        var message = $"Error when convert record - codigoproduto: {records[i].Where(pair => pair.Key == "codigoproduto").Select(pair => pair.Value).FirstOrDefault()} | deposito: {records[i].Where(pair => pair.Key == "deposito").Select(pair => pair.Value).FirstOrDefault()} ";
                    ...
                    list.Add(new B2CConsultaProdutosDetalhesDepositos(entity, xml));
                }
                catch (Exception ex)
                {
                    throw new GeneralException(
                        message: $"Error when convert record - codigoproduto: {records[i].Where(pair => pair.Key == "codigoproduto").Select(pair => pair.Value).FirstOrDefault()} | deposito: {records[i].Where(pair => pair.Key == "deposito").Select(pair => pair.Value).FirstOrDefault()} - {ex.Message}",
        ...
        public async Task<bool> GetRecords(LinxAPIParam jobParameter)
        {
            ...
            foreach (var cnpj_emp in cnpjs_emp)
            {
                ...
                if (xmls.Count() > 0)
                {
                    ...
                    if (_b2cConsultaProdutosDetalhesDepositosCache.Count == 0)
                    {
                        ...
                        _b2cConsultaProdutosDetalhesDepositosCache = list_existentes.ToList();
                    }

                    var _listSomenteNovos = listRecords.Where(x => !_b2cConsultaProdutosDetalhesDepositosCache.Any(y =>
                        y == x.recordKey
                    )).ToList();
                    ...
                    if (_listSomenteNovos.Count() > 0)
                    {
                        ...
                        _b2cConsultaProdutosDetalhesDepositosCache.AddRange(_listSomenteNovos.Select(x => x.recordKey));
                    }
