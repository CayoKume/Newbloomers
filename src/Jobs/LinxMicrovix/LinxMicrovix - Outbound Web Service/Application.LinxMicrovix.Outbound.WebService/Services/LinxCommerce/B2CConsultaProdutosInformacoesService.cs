            ...
            for (int i = 0; i < records.Count(); i++)
            {
                ...
                catch (Exception ex)
                {
                    throw new GeneralException(
                        message: $"Error when convert record - codigoproduto: {records[i].Where(pair => pair.Key == " - {ex.Message}codigoproduto").Select(pair => pair.Value).FirstOrDefault()} | informacoes_produto: {records[i].Where(pair => pair.Key == "informacoes_produto").Select(pair => pair.Value).FirstOrDefault()} - {ex.Message}",
