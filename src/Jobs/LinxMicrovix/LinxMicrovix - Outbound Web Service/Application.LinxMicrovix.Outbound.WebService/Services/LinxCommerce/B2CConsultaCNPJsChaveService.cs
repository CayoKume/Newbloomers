...
namespace Application.LinxMicrovix.Outbound.WebService.Services
{
    public class B2CConsultaCNPJsChaveService : IB2CConsultaCNPJsChaveService
    {
        ...
        private readonly IValidator<Domain.LinxMicrovix.Outbound.WebService.Dtos.LinxCommerce.B2CConsultaCNPJsChave> _validator;
        ...
        public B2CConsultaCNPJsChaveService(
            ...
            IValidator<Domain.LinxMicrovix.Outbound.WebService.Dtos.LinxCommerce.B2CConsultaCNPJsChave> validator,
        ...
        public List<B2CConsultaCNPJsChave?> DeserializeXMLToObject(LinxAPIParam jobParameter, List<Dictionary<string?, string?>> records)
        {
            ...
            for (int i = 0; i < records.Count(); i++)
            {
                try
                {
                    ...
                    {
                        var message = $"Error when convert record - cnpj: {records[i].Where(pair => pair.Key == ";
                    ...
                    list.Add(new B2CConsultaCNPJsChave(entity, xml));
                }
                catch (Exception ex)
                {
                    throw new GeneralException(
                        message: $"Error when convert record - cnpj: {records[i].Where(pair => pair.Key == " - {ex.Message}cnpj").Select(pair => pair.Value).FirstOrDefault()} | nome_empresa: {records[i].Where(pair => pair.Key == "nome_empresa").Select(pair => pair.Value).FirstOrDefault()} - {ex.Message}",
