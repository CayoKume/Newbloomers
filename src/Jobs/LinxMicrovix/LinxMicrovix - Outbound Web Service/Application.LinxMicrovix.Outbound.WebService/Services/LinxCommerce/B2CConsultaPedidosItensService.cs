            ...
            for (int i = 0; i < records.Count(); i++)
            {
                ...
                catch (Exception ex)
                {
                    throw new GeneralException(
                        message: $"Error when convert record - id_pedido_item: {records[i].Where(pair => pair.Key == " - {ex.Message}id_pedido_item").Select(pair => pair.Value).FirstOrDefault()} | codigoproduto: {records[i].Where(pair => pair.Key == "codigoproduto").Select(pair => pair.Value).FirstOrDefault()} - {ex.Message}",
