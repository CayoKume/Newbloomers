...
namespace Application.LinxMicrovix.Outbound.WebService.Services
{
    public class B2CConsultaProdutosDetalhesService : IB2CConsultaProdutosDetalhesService
    {
        ...
        private readonly IValidator<Domain.LinxMicrovix.Outbound.WebService.Dtos.LinxCommerce.B2CConsultaProdutosDetalhes> _validator;
        private static List<string?> _b2cConsultaProdutosDetalhesCache { get; set; } = new List<string?>();
        ...
        public B2CConsultaProdutosDetalhesService(
            ...
            IValidator<Domain.LinxMicrovix.Outbound.WebService.Dtos.LinxCommerce.B2CConsultaProdutosDetalhes> validator,
        ...
        public List<B2CConsultaProdutosDetalhes?> DeserializeXMLToObject(LinxAPIParam jobParameter, List<Dictionary<string?, string?>> records)
        {
            ...
            for (int i = 0; i < records.Count(); i++)
            {
                try
                {
                    ...
                    var entity = new Domain.LinxMicrovix.Outbound.WebService.Dtos.LinxCommerce.B2CConsultaProdutosDetalhes(
                    ...
                    {
                        var message = $"Error when convert record - id_prod_det: {records[i].Where(pair => pair.Key == ";
                    ...
                    list.Add(new B2CConsultaProdutosDetalhes(entity, xml));
                }
                catch (Exception ex)
                {
                    throw new GeneralException(
                        message: $"Error when convert record - id_prod_det: {records[i].Where(pair => pair.Key == " - {ex.Message}id_prod_det").Select(pair => pair.Value).FirstOrDefault()} | codigoproduto: {records[i].Where(pair => pair.Key == "codigoproduto").Select(pair => pair.Value).FirstOrDefault()} - {ex.Message}",
