        ...
        public List<B2CConsultaImagens?> DeserializeXMLToObject(LinxAPIParam jobParameter, List<Dictionary<string?, string?>> records)
        {
            ...
            for (int i = 0; i < records.Count(); i++)
            {
                try
                {
                    ...
                    {
                        var message = $"Error when convert record - id_imagem: {records[i].Where(pair => pair.Key == ";
                ...
                catch (Exception ex)
                {
                    throw new GeneralException(
                        message: $"Error when convert record - id_imagem: {records[i].Where(pair => pair.Key == " - {ex.Message}id_imagem").Select(pair => pair.Value).FirstOrDefault()} | url_imagem_blob: {records[i].Where(pair => pair.Key == "url_imagem_blob").Select(pair => pair.Value).FirstOrDefault()} - {ex.Message}",
