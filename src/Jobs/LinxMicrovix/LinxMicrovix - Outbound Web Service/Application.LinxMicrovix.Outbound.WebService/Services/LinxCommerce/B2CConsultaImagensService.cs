...
namespace Application.LinxMicrovix.Outbound.WebService.Services
{
    public class B2CConsultaImagensService : IB2CConsultaImagensService
    {
        ...
        private readonly IValidator<Domain.LinxMicrovix.Outbound.WebService.Dtos.LinxCommerce.B2CConsultaImagens> _validator;
        private static List<string?> _b2cConsultaImagensCache { get; set; } = new List<string?>();
        ...
        public B2CConsultaImagensService(
            ...
            IValidator<Domain.LinxMicrovix.Outbound.WebService.Dtos.LinxCommerce.B2CConsultaImagens> validator,
        ...
        public List<B2CConsultaImagens?> DeserializeXMLToObject(LinxAPIParam jobParameter, List<Dictionary<string?, string?>> records)
        {
            ...
            for (int i = 0; i < records.Count(); i++)
            {
                try
                {
                    ...
                    {
                        var message = $"Error when convert record - id_imagem: {records[i].Where(pair => pair.Key == "id_imagem").Select(pair => pair.Value).FirstOrDefault()} | url_imagem_blob: {records[i].Where(pair => pair.Key == "url_imagem_blob").Select(pair => pair.Value).FirstOrDefault()} ";
                    ...
                    list.Add(new B2CConsultaImagens(entity, xml));
                }
                catch (Exception ex)
                {
                    throw new GeneralException(
                        message: $"Error when convert record - id_imagem: {records[i].Where(pair => pair.Key == "id_imagem").Select(pair => pair.Value).FirstOrDefault()} | url_imagem_blob: {records[i].Where(pair => pair.Key == "url_imagem_blob").Select(pair => pair.Value).FirstOrDefault()} - {ex.Message}",
        ...
        public async Task<bool> GetRecord(LinxAPIParam jobParameter, string? identificador, string? cnpj_emp)
        {
            _logger
               .Clear()
               .AddLog(EnumJob.B2CConsultaImagens);
        ...
        public async Task<bool> GetRecords(LinxAPIParam jobParameter)
        {
            ...
            foreach (var cnpj_emp in cnpjs_emp)
            {
                ...
                if (xmls.Count() > 0)
                {
                    ...
                    if (_b2cConsultaImagensCache.Count == 0)
                    {
                        var list_existentes = await _b2cConsultaImagensRepository.GetRegistersExists(jobParameter: jobParameter, registros: listRecords);
                        _b2cConsultaImagensCache = list_existentes.ToList();
                    }

                    var _listSomenteNovos = listRecords.Where(x => !_b2cConsultaImagensCache.Any(y =>
                        y == x.recordKey
                    )).ToList();

                    if (_listSomenteNovos.Count() > 0)
                    {
                        _b2cConsultaImagensRepository.BulkInsertIntoTableRaw(records: _listSomenteNovos, jobParameter: jobParameter);
                        await _coreRepository.CallDbProcMerge(jobParameter.schema, jobParameter.tableName, _logger.GetExecutionGuid());
                        ...
                        _b2cConsultaImagensCache.AddRange(_listSomenteNovos.Select(x => x.recordKey));
                    }
