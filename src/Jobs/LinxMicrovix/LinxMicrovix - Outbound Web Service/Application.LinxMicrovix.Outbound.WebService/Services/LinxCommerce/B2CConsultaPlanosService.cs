...
namespace Application.LinxMicrovix.Outbound.WebService.Services
{
    public class B2CConsultaPlanosService : IB2CConsultaPlanosService
    {
        ...
        private readonly IValidator<Domain.LinxMicrovix.Outbound.WebService.Dtos.LinxCommerce.B2CConsultaPlanos> _validator;
        ...
        public B2CConsultaPlanosService(
            ...
            IValidator<Domain.LinxMicrovix.Outbound.WebService.Dtos.LinxCommerce.B2CConsultaPlanos> validator,
        ...
        public List<B2CConsultaPlanos?> DeserializeXMLToObject(LinxAPIParam jobParameter, List<Dictionary<string?, string?>> records)
        {
            ...
            for (int i = 0; i < records.Count(); i++)
            {
                try
                {
                    ...
                    var entity = new Domain.LinxMicrovix.Outbound.WebService.Dtos.LinxCommerce.B2CConsultaPlanos(
                    ...
                    list.Add(new B2CConsultaPlanos(entity, xml));
                }
                catch (Exception ex)
                {
                    throw new GeneralException(
                        message: $"Error when convert record - plano: {records[i].Where(pair => pair.Key == " - {ex.Message}plano").Select(pair => pair.Value).FirstOrDefault()} | nome_plano: {records[i].Where(pair => pair.Key == "nome_plano").Select(pair => pair.Value).FirstOrDefault()} - {ex.Message}",
