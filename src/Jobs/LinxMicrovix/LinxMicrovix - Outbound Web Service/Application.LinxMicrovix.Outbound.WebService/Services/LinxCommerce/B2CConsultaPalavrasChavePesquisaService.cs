            ...
            for (int i = 0; i < records.Count(); i++)
            {
                ...
                catch (Exception ex)
                {
                    throw new GeneralException(
                        message: $"Error when convert record - id_b2c_palavras_chave_pesquisa: {records[i].Where(pair => pair.Key == " - {ex.Message}id_b2c_palavras_chave_pesquisa").Select(pair => pair.Value).FirstOrDefault()} | nome_colecao: {records[i].Where(pair => pair.Key == "nome_colecao").Select(pair => pair.Value).FirstOrDefault()} - {ex.Message}",
