...
namespace Application.LinxMicrovix.Outbound.WebService.Services
{
    public class B2CConsultaPalavrasChavePesquisaService : IB2CConsultaPalavrasChavePesquisaService
    {
        ...
        private readonly IValidator<Domain.LinxMicrovix.Outbound.WebService.Dtos.LinxCommerce.B2CConsultaPalavrasChavePesquisa> _validator;
        ...
        public B2CConsultaPalavrasChavePesquisaService(
            ...
            IValidator<Domain.LinxMicrovix.Outbound.WebService.Dtos.LinxCommerce.B2CConsultaPalavrasChavePesquisa> validator,
        ...
        public List<B2CConsultaPalavrasChavePesquisa?> DeserializeXMLToObject(LinxAPIParam jobParameter, List<Dictionary<string?, string?>> records)
        {
            ...
            for (int i = 0; i < records.Count(); i++)
            {
                try
                {
                    var entity = new Domain.LinxMicrovix.Outbound.WebService.Dtos.LinxCommerce.B2CConsultaPalavrasChavePesquisa(
                    ...
                    {
                        var message = $"Error when convert record - id_b2c_palavras_chave_pesquisa: {records[i].Where(pair => pair.Key == "id_b2c_palavras_chave_pesquisa").Select(pair => pair.Value).FirstOrDefault()} | nome_colecao: {records[i].Where(pair => pair.Key == "nome_colecao").Select(pair => pair.Value).FirstOrDefault()} ";
                    ...
                    list.Add(new B2CConsultaPalavrasChavePesquisa(entity, xml));
                }
                catch (Exception ex)
                {
                    throw new GeneralException(
                        message: $"Error when convert record - id_b2c_palavras_chave_pesquisa: {records[i].Where(pair => pair.Key == "id_b2c_palavras_chave_pesquisa").Select(pair => pair.Value).FirstOrDefault()} | nome_colecao: {records[i].Where(pair => pair.Key == "nome_colecao").Select(pair => pair.Value).FirstOrDefault()} - {ex.Message}",
        ...
        public async Task<bool> GetRecords(LinxAPIParam jobParameter)
        {
            ...
            foreach (var cnpj_emp in cnpjs_emp)
            {
                ...
                if (xmls.Count() > 0)
                {
                    ...
                    if (_b2cConsultaPalavrasChavePesquisaCache.Count == 0)
                    {
                        ...
                        _b2cConsultaPalavrasChavePesquisaCache = list_existentes.ToList();
                    }

                    var _listSomenteNovos = listRecords.Where(x => !_b2cConsultaPalavrasChavePesquisaCache.Any(y =>
                        y == x.recordKey
                    )).ToList();
                    ...
                    if (_listSomenteNovos.Count() > 0)
                    {
                        ...
                        _b2cConsultaPalavrasChavePesquisaCache.AddRange(_listSomenteNovos.Select(x => x.recordKey));
                    }
