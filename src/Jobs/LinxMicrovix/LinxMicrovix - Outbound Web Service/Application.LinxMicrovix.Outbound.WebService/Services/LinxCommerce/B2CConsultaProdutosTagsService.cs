...
namespace Application.LinxMicrovix.Outbound.WebService.Services
{
    public class B2CConsultaProdutosTagsService : IB2CConsultaProdutosTagsService
    {
        ...
        private readonly IValidator<Domain.LinxMicrovix.Outbound.WebService.Dtos.LinxCommerce.B2CConsultaProdutosTags> _validator;
        ...
        public B2CConsultaProdutosTagsService(
            ...
            IValidator<Domain.LinxMicrovix.Outbound.WebService.Dtos.LinxCommerce.B2CConsultaProdutosTags> validator,
        ...
        public List<B2CConsultaProdutosTags?> DeserializeXMLToObject(LinxAPIParam jobParameter, List<Dictionary<string?, string?>> records)
        {
            ...
            for (int i = 0; i < records.Count(); i++)
            {
                try
                {
                    var entity = new Domain.LinxMicrovix.Outbound.WebService.Dtos.LinxCommerce.B2CConsultaProdutosTags(
                    ...
                    {
                        var message = $"Error when convert record - id_b2c_tags_produtos: {records[i].Where(pair => pair.Key == "id_b2c_tags_produtos").Select(pair => pair.Value).FirstOrDefault()} | descricao_b2c_tags: {records[i].Where(pair => pair.Key == "descricao_b2c_tags").Select(pair => pair.Value).FirstOrDefault()} ";
                    ...
                    list.Add(new B2CConsultaProdutosTags(entity, xml));
                }
                catch (Exception ex)
                {
                    throw new GeneralException(
                        message: $"Error when convert record - id_b2c_tags_produtos: {records[i].Where(pair => pair.Key == "id_b2c_tags_produtos").Select(pair => pair.Value).FirstOrDefault()} | descricao_b2c_tags: {records[i].Where(pair => pair.Key == "descricao_b2c_tags").Select(pair => pair.Value).FirstOrDefault()} - {ex.Message}",
        ...
        public async Task<bool> GetRecords(LinxAPIParam jobParameter)
        {
            ...
            foreach (var cnpj_emp in cnpjs_emp)
            {
                ...
                if (xmls.Count() > 0)
                {
                    ...
                    if (_b2cConsultaProdutosTagsCache.Count == 0)
                    {
                        ...
                        _b2cConsultaProdutosTagsCache = list_existentes.ToList();
                    }

                    var _listSomenteNovos = listRecords.Where(x => !_b2cConsultaProdutosTagsCache.Any(y =>
                        y == x.recordKey
                    )).ToList();
                    ...
                    if (_listSomenteNovos.Count() > 0)
                    {
                        ...
                        _b2cConsultaProdutosTagsCache.AddRange(_listSomenteNovos.Select(x => x.recordKey));
                    }
