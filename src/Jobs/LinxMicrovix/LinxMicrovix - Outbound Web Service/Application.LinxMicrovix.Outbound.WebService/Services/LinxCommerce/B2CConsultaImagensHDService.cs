...
namespace Application.LinxMicrovix.Outbound.WebService.Services
{
    public class B2CConsultaImagensHDService : IB2CConsultaImagensHDService
    {
        ...
        private readonly IValidator<Domain.LinxMicrovix.Outbound.WebService.Dtos.LinxCommerce.B2CConsultaImagensHD> _validator;
        ...
        public B2CConsultaImagensHDService(
            ...
            IValidator<Domain.LinxMicrovix.Outbound.WebService.Dtos.LinxCommerce.B2CConsultaImagensHD> validator,
        ...
        public List<B2CConsultaImagensHD?> DeserializeXMLToObject(LinxAPIParam jobParameter, List<Dictionary<string?, string?>> records)
        {
            ...
            for (int i = 0; i < records.Count(); i++)
            {
                try
                {
                    ...
                    var entity = new Domain.LinxMicrovix.Outbound.WebService.Dtos.LinxCommerce.B2CConsultaImagensHD(
                    ...
                    list.Add(new B2CConsultaImagensHD(entity, xml));
                }
                catch (Exception ex)
                {
                    throw new GeneralException(
                        message: $"Error when convert record - codigoproduto: {records[i].Where(pair => pair.Key == " - {ex.Message}codigoproduto").Select(pair => pair.Value).FirstOrDefault()} | identificador_imagem: {records[i].Where(pair => pair.Key == "identificador_imagem").Select(pair => pair.Value).FirstOrDefault()} - {ex.Message}",
