...
namespace Application.LinxMicrovix.Outbound.WebService.Services
{
    public class B2CConsultaProdutosAssociadosService : IB2CConsultaProdutosAssociadosService
    {
        ...
        private readonly IValidator<Domain.LinxMicrovix.Outbound.WebService.Dtos.LinxCommerce.B2CConsultaProdutosAssociados> _validator;
        ...
        public B2CConsultaProdutosAssociadosService(
            ...
            IValidator<Domain.LinxMicrovix.Outbound.WebService.Dtos.LinxCommerce.B2CConsultaProdutosAssociados> validator,
        ...
        public List<B2CConsultaProdutosAssociados?> DeserializeXMLToObject(LinxAPIParam jobParameter, List<Dictionary<string?, string?>> records)
        {
            ...
            for (int i = 0; i < records.Count(); i++)
            {
                try
                {
                    ...
                    list.Add(new B2CConsultaProdutosAssociados(entity, xml));
                }
                catch (Exception ex)
                {
                    throw new GeneralException(
                        message: $"Error when convert record - codigoproduto: {records[i].Where(pair => pair.Key == " - {ex.Message}codigoproduto").Select(pair => pair.Value).FirstOrDefault()} | codigoproduto_associado: {records[i].Where(pair => pair.Key == "codigoproduto_associado").Select(pair => pair.Value).FirstOrDefault()} - {ex.Message}",
