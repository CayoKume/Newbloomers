...
namespace Application.LinxMicrovix.Outbound.WebService.Services
{
    public class B2CConsultaProdutosCamposAdicionaisNomesService : IB2CConsultaProdutosCamposAdicionaisNomesService
    {
        ...
        private readonly IValidator<Domain.LinxMicrovix.Outbound.WebService.Dtos.LinxCommerce.B2CConsultaProdutosCamposAdicionaisNomes> _validator;
        ...
        public B2CConsultaProdutosCamposAdicionaisNomesService(
            ...
            IValidator<Domain.LinxMicrovix.Outbound.WebService.Dtos.LinxCommerce.B2CConsultaProdutosCamposAdicionaisNomes> validator,
        ...
        public List<B2CConsultaProdutosCamposAdicionaisNomes?> DeserializeXMLToObject(LinxAPIParam jobParameter, List<Dictionary<string?, string?>> records)
        {
            ...
            for (int i = 0; i < records.Count(); i++)
            {
                try
                {
                    var entity = new Domain.LinxMicrovix.Outbound.WebService.Dtos.LinxCommerce.B2CConsultaProdutosCamposAdicionaisNomes(
                    ...
                    {
                        var message = $"Error when convert record - id_campo: {records[i].Where(pair => pair.Key == "id_campo").Select(pair => pair.Value).FirstOrDefault()} | legenda: {records[i].Where(pair => pair.Key == "legenda").Select(pair => pair.Value).FirstOrDefault()} ";
                    ...
                    list.Add(new B2CConsultaProdutosCamposAdicionaisNomes(entity, xml));
                }
                catch (Exception ex)
                {
                    throw new GeneralException(
                        message: $"Error when convert record - id_campo: {records[i].Where(pair => pair.Key == "id_campo").Select(pair => pair.Value).FirstOrDefault()} | legenda: {records[i].Where(pair => pair.Key == "legenda").Select(pair => pair.Value).FirstOrDefault()} - {ex.Message}",
        ...
        public async Task<bool> GetRecords(LinxAPIParam jobParameter)
        {
            ...
            foreach (var cnpj_emp in cnpjs_emp)
            {
                ...
                if (xmls.Count() > 0)
                {
                    ...
                    if (_b2cConsultaProdutosCamposAdicionaisNomesCache.Count == 0)
                    {
                        ...
                        _b2cConsultaProdutosCamposAdicionaisNomesCache = list_existentes.ToList();
                    }

                    var _listSomenteNovos = listRecords.Where(x => !_b2cConsultaProdutosCamposAdicionaisNomesCache.Any(y =>
                        y == x.recordKey
                    )).ToList();
                    ...
                    if (_listSomenteNovos.Count() > 0)
                    {
                        ...
                        _b2cConsultaProdutosCamposAdicionaisNomesCache.AddRange(_listSomenteNovos.Select(x => x.recordKey));
                    }
