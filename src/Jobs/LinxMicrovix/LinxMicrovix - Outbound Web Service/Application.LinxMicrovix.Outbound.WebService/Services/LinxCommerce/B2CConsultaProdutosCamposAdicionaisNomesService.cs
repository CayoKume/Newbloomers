        ...
        public List<B2CConsultaProdutosCamposAdicionaisNomes?> DeserializeXMLToObject(LinxAPIParam jobParameter, List<Dictionary<string?, string?>> records)
        {
            ...
            for (int i = 0; i < records.Count(); i++)
            {
                try
                {
                    ...
                    {
                        var message = $"Error when convert record - id_campo: {records[i].Where(pair => pair.Key == ";
                ...
                catch (Exception ex)
                {
                    throw new GeneralException(
                        message: $"Error when convert record - id_campo: {records[i].Where(pair => pair.Key == " - {ex.Message}id_campo").Select(pair => pair.Value).FirstOrDefault()} | legenda: {records[i].Where(pair => pair.Key == "legenda").Select(pair => pair.Value).FirstOrDefault()} - {ex.Message}",
