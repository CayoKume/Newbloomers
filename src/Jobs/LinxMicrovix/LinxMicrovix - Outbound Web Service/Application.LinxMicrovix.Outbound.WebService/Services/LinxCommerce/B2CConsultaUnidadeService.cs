        ...
        public List<B2CConsultaUnidade?> DeserializeXMLToObject(LinxAPIParam jobParameter, List<Dictionary<string?, string?>> records)
        {
            ...
            for (int i = 0; i < records.Count(); i++)
            {
                try
                {
                    ...
                    {
                        var message = $"Error when convert record - unidade: {records[i].Where(pair => pair.Key == ";
                ...
                catch (Exception ex)
                {
                    throw new GeneralException(
                        message: $"Error when convert record - unidade: {records[i].Where(pair => pair.Key == " - {ex.Message}unidade").Select(pair => pair.Value).FirstOrDefault()} | timestamp: {records[i].Where(pair => pair.Key == "timestamp").Select(pair => pair.Value).FirstOrDefault()} - {ex.Message}",
