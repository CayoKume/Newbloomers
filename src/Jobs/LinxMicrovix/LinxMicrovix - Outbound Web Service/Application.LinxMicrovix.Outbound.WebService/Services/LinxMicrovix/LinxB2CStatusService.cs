...
namespace Application.LinxMicrovix.Outbound.WebService.Services.LinxMicrovix
{
    public class LinxB2CStatusService : ILinxB2CStatusService
    {
        ...
        private readonly IValidator<Domain.LinxMicrovix.Outbound.WebService.Dtos.LinxMicrovix.LinxB2CStatus> _validator;
        ...
        public LinxB2CStatusService(
            ...
            IValidator<Domain.LinxMicrovix.Outbound.WebService.Dtos.LinxMicrovix.LinxB2CStatus> validator,
        ...
        public List<LinxB2CStatus?> DeserializeXMLToObject(LinxAPIParam jobParameter, List<Dictionary<string?, string?>> records)
        {
            ...
            for (int i = 0; i < records.Count(); i++)
            {
                try
                {
                    var entity = new Domain.LinxMicrovix.Outbound.WebService.Dtos.LinxMicrovix.LinxB2CStatus(
                    ...
                    {
                        var message = $"Error when convert record - id_status: {records[i].Where(pair => pair.Key == "id_status").Select(pair => pair.Value).FirstOrDefault()} | descricao_status: {records[i].Where(pair => pair.Key == "descricao_status").Select(pair => pair.Value).FirstOrDefault()}" + "\n";
                    ...
                    list.Add(new LinxB2CStatus(entity, xml));
                }
                catch (Exception ex)
                {
