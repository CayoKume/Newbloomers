...
namespace Application.LinxMicrovix.Outbound.WebService.Services.LinxMicrovix
{
    public class LinxXMLDocumentosService : ILinxXMLDocumentosService
    {
        ...
        private readonly IValidator<Domain.LinxMicrovix.Outbound.WebService.Dtos.LinxMicrovix.LinxXMLDocumentos> _validator;
        ...
        public LinxXMLDocumentosService(
            ...
            IValidator<Domain.LinxMicrovix.Outbound.WebService.Dtos.LinxMicrovix.LinxXMLDocumentos> validator,
        ...
        public List<LinxXMLDocumentos?> DeserializeXMLToObject(LinxAPIParam jobParameter, List<Dictionary<string?, string?>> records)
        {
            ...
            for (int i = 0; i < records.Count(); i++)
            {
                try
                {
                    var entity = new Domain.LinxMicrovix.Outbound.WebService.Dtos.LinxMicrovix.LinxXMLDocumentos(
                    ...
                    {
                        var message = $"Error when convert record - documento: {records[i].Where(pair => pair.Key == "documento").Select(pair => pair.Value).FirstOrDefault()} | chave_nfe: {records[i].Where(pair => pair.Key == "chave_nfe").Select(pair => pair.Value).FirstOrDefault()}" + "\n";
                    ...
                    list.Add(new LinxXMLDocumentos(entity, xml));
                }
                catch (Exception ex)
                {
        ...
        public async Task<bool> GetRecords(LinxAPIParam jobParameter)
        {
            ...
            if (xmls.Count() > 0)
            {
                ...
                if (_listSomenteNovos.Count() > 0)
                {
                    ...
                    _linxXMLDocumentosCache.AddRange(_listSomenteNovos.Select(x => x.recordKey));
                }
