...
namespace Application.LinxMicrovix.Outbound.WebService.Services.LinxMicrovix
{
    public class LinxProdutosCamposAdicionaisService : ILinxProdutosCamposAdicionaisService
    {
        ...
        private readonly IValidator<Domain.LinxMicrovix.Outbound.WebService.Dtos.LinxMicrovix.LinxProdutosCamposAdicionais> _validator;
        ...
        public LinxProdutosCamposAdicionaisService(
            ...
            IValidator<Domain.LinxMicrovix.Outbound.WebService.Dtos.LinxMicrovix.LinxProdutosCamposAdicionais> validator,
        ...
        public List<LinxProdutosCamposAdicionais?> DeserializeXMLToObject(LinxAPIParam jobParameter, List<Dictionary<string?, string?>> records)
        {
            ...
            for (int i = 0; i < records.Count(); i++)
            {
                try
                {
                    var entity = new Domain.LinxMicrovix.Outbound.WebService.Dtos.LinxMicrovix.LinxProdutosCamposAdicionais(
                    ...
                    {
                        var message = $"Error when convert record - cod_produto: {records[i].Where(pair => pair.Key == "cod_produto").Select(pair => pair.Value).FirstOrDefault()} | campo: {records[i].Where(pair => pair.Key == "campo").Select(pair => pair.Value).FirstOrDefault()} | valor: {records[i].Where(pair => pair.Key == "valor").Select(pair => pair.Value).FirstOrDefault()}" + "\n";
                    ...
                    list.Add(new LinxProdutosCamposAdicionais(entity, xml));
                }
                catch (Exception ex)
                {
