...
{
    public class LinxMetasVendedoresDiaValidator : AbstractValidator<LinxMetasVendedoresDia>
    {
        public LinxMetasVendedoresDiaValidator()
        {
            ...
            RuleFor(x => x.dia).MustBeValidDateTime().When(x => !string.IsNullOrEmpty(x.dia));
            RuleFor(x => x.valor_meta_diaria).MustBeValidDecimal().When(x => !string.IsNullOrEmpty(x.valor_meta_diaria));
