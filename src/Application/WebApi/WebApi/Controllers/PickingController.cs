...
namespace Controllers
{
    ...
    public class PickingController : ControllerBase
    {
        ...
        public async Task<ActionResult> UpdateShippingCompany([FromBody] UpdateShippingCompanyRequest request)
        {
            try
            {
                var result = await _pickingService.UpdateShippingCompany(request.orderNumber, Convert.ToInt32(request.cod_shippingCompany));
    ...
    public class UpdateRetornoRequest
    {
        ...
        public List<PickingProduct>? itens { get; set; } //Aqui
