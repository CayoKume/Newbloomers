...
namespace Infrastructure.DatabaseInit.Repositorys.LinxMicrovix.LinxMicrovix
{
    public class LinxMetasVendedoresDiaRepository : ILinxMetasVendedoresDiaRepository
    {
        ...
        public LinxMetasVendedoresDiaRepository(ISQLServerConnection? conn) =>
            _conn = conn;
        ...
        public bool CreateTableIfNotExists(string databaseName, string jobName, string untreatedDatabaseName)
        {
            ...
            try
            {
                using (var conn = _conn.GetIDbConnection(databaseName))
                {
                    ...
                    if (result.Count() == 0)
                        conn.CreateTable<Domain.DatabaseInit.Entities.LinxMicrovix.Trusted.LinxMicrovix.LinxMetasVendedoresDia>();
                }
            }
            catch (Exception ex)
            {
            ...
            try
            {
                using (var conn = _conn.GetIDbConnection(untreatedDatabaseName))
                {
                    ...
                    if (result.Count() == 0)
                        conn.CreateTable<Domain.DatabaseInit.Entities.LinxMicrovix.Untreated.LinxMicrovix.LinxMetasVendedoresDia>();
                }
            }
            catch (Exception ex)
            {
        ...
        public async Task<bool> InsertParametersIfNotExists(string jobName, string parametersTableName, string databaseName)
        {
            try
            {
                ...
                {
                    method = jobName,
                    timestamp = @"<Parameter id=""data_inicial_meta"">[data_inicial_meta]</Parameter>
                                     <Parameter id=""data_fim_meta"">[data_fim_meta]</Parameter>",
                    dateinterval = @"<Parameter id=""data_inicial_meta"">[data_inicial_meta]</Parameter>
                                     <Parameter id=""data_fim_meta"">[data_fim_meta]</Parameter>",
                    individual = @"<Parameter id=""data_inicial_meta"">[data_inicial_meta]</Parameter>
                                    <Parameter id=""data_fim_meta"">[data_fim_meta]</Parameter>
