    ...
    public class LinxProdutosInventarioRepository : ILinxProdutosInventarioRepository
    {
        ...
        public bool CreateTableIfNotExists(string databaseName, string jobName, string untreatedDatabaseName)
        {
            ...
            try
            {
                using (var conn = _conn.GetIDbConnection(databaseName))
                {
                    ...
                    if (result.Count() == 0)
                        conn.CreateTable<Domain.DatabaseInit.Entities.LinxMicrovix.Trusted.LinxMicrovix.LinxProdutosInventario>();
                }
            }
            catch (Exception ex)
            {
            ...
            try
            {
                using (var conn = _conn.GetIDbConnection(untreatedDatabaseName))
                {
                    ...
                    if (result.Count() == 0)
                        conn.CreateTable<Domain.DatabaseInit.Entities.LinxMicrovix.Untreated.LinxMicrovix.LinxProdutosInventario>();
                }
            }
            catch (Exception ex)
            {
        ...
        public async Task<bool> InsertParametersIfNotExists(string jobName, string parametersTableName, string databaseName)
        {
            try
            {
                ...
                {
                    method = jobName,
                    timestamp = @"<Parameter id=""data_inventario"">[data_inventario]</Parameter>",
                    dateinterval = @"<Parameter id=""data_inventario"">[data_inventario]</Parameter>",
                    individual = @"<Parameter id=""data_inventario"">[data_inventario]</Parameter>
                                                <Parameter id=""cod_produto"">[cod_produto]</Parameter>
                                                <Parameter id=""cod_deposito"">[cod_deposito]</Parameter>"
                };
