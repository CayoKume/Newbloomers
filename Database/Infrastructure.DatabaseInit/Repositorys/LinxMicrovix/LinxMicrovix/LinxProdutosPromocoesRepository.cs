...
namespace Infrastructure.DatabaseInit.Repositorys.LinxMicrovix.LinxMicrovix
{
    public class LinxProdutosPromocoesRepository : ILinxProdutosPromocoesRepository
    {
        ...
        public LinxProdutosPromocoesRepository(ISQLServerConnection? conn) =>
            _conn = conn;
        ...
        public bool CreateTableIfNotExists(string databaseName, string jobName, string untreatedDatabaseName)
        {
            ...
            try
            {
                using (var conn = _conn.GetIDbConnection(databaseName))
                {
                    ...
                    if (result.Count() == 0)
                        conn.CreateTable<Domain.DatabaseInit.Entities.LinxMicrovix.Trusted.LinxMicrovix.LinxProdutosPromocoes>();
                }
            }
            catch (Exception ex)
            {
            ...
            try
            {
                using (var conn = _conn.GetIDbConnection(untreatedDatabaseName))
                {
                    ...
                    if (result.Count() == 0)
                        conn.CreateTable<Domain.DatabaseInit.Entities.LinxMicrovix.Untreated.LinxMicrovix.LinxProdutosPromocoes>();
                }
            }
            catch (Exception ex)
            {
            ...
            try
            {
                var parameter = new
                {
                    ...
                    timestamp = @"<Parameter id=""timestamp"">[0]</Parameter>
                                                <Parameter id=""data_cad_inicial"">[data_cad_inicial]</Parameter>
                                                <Parameter id=""data_cad_fim"">[data_cad_fim]</Parameter>
                                                <Parameter id=""data_vig_inicial"">[data_vig_inicial]</Parameter>
                                                <Parameter id=""data_vig_fim"">[data_vig_fim]</Parameter>
                                                <Parameter id=""promocao_ativa"">[promocao_ativa]</Parameter>",
