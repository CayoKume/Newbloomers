...
namespace Infrastructure.DatabaseInit.Repositorys.LinxMicrovix.LinxMicrovix
{
    public class LinxLancContabilRepository : ILinxLancContabilRepository
    {
        ...
        public LinxLancContabilRepository(ISQLServerConnection? conn) =>
            _conn = conn;
        ...
        public bool CreateTableIfNotExists(string databaseName, string jobName, string untreatedDatabaseName)
        {
            ...
            try
            {
                using (var conn = _conn.GetIDbConnection(databaseName))
                {
                    ...
                    if (result.Count() == 0)
                        conn.CreateTable<Domain.DatabaseInit.Entities.LinxMicrovix.Trusted.LinxMicrovix.LinxLancContabil>();
                }
            }
            catch (Exception ex)
            {
            ...
            try
            {
                using (var conn = _conn.GetIDbConnection(untreatedDatabaseName))
                {
                    ...
                    if (result.Count() == 0)
                        conn.CreateTable<Domain.DatabaseInit.Entities.LinxMicrovix.Untreated.LinxMicrovix.LinxLancContabil>();
                }
            }
            catch (Exception ex)
            {
            ...
            try
            {
                var parameter = new
                {
                    ...
                    timestamp = @"<Parameter id=""timestamp"">[0]</Parameter>
                                  <Parameter id=""data_lanc_ini"">[data_lanc_ini]</Parameter>
                                  <Parameter id=""data_lanc_fim"">[data_lanc_fim]</Parameter>
                                  <Parameter id=""cod_cc"">[cod_cc]</Parameter>
                                  <Parameter id=""cod_conta"">[cod_conta]</Parameter>",
                    dateinterval = @"<Parameter id=""data_lanc_ini"">[data_lanc_ini]</Parameter>
                                     <Parameter id=""data_lanc_fim"">[data_lanc_fim]</Parameter>
                                     <Parameter id=""cod_cc"">[cod_cc]</Parameter>
                                     <Parameter id=""cod_conta"">[cod_conta]</Parameter>",
                    individual = @"<Parameter id=""data_lanc_ini"">[data_lanc_ini]</Parameter>
                                   <Parameter id=""data_lanc_fim"">[data_lanc_fim]</Parameter>
                                   <Parameter id=""cod_cc"">[cod_cc]</Parameter>
                                   <Parameter id=""cod_conta"">[cod_conta]</Parameter>
