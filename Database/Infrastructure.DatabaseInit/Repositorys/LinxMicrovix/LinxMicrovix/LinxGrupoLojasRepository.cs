    ...
    public class LinxGrupoLojasRepository : ILinxGrupoLojasRepository
    {
        ...
        public bool CreateTableIfNotExists(string databaseName, string jobName, string untreatedDatabaseName)
        {
            ...
            try
            {
                using (var conn = _conn.GetIDbConnection(databaseName))
                {
                    ...
                    if (result.Count() == 0)
                        conn.CreateTable<Domain.DatabaseInit.Entities.LinxMicrovix.Trusted.LinxMicrovix.LinxGrupoLojas>();
                }
            }
            catch (Exception ex)
            {
            ...
            try
            {
                using (var conn = _conn.GetIDbConnection(untreatedDatabaseName))
                {
                    ...
                    if (result.Count() == 0)
                        conn.CreateTable<Domain.DatabaseInit.Entities.LinxMicrovix.Untreated.LinxMicrovix.LinxGrupoLojas>();
                }
            }
            catch (Exception ex)
            {
        ...
        public async Task<bool> InsertParametersIfNotExists(string jobName, string parametersTableName, string databaseName)
        {
            try
            {
                ...
                {
                    method = jobName,
                    timestamp = @"<Parameter id=""portal"">[0]</Parameter>",
                    dateinterval = @"<Parameter id=""portal"">[0]</Parameter>",
                    individual = @"<Parameter id=""id_empresas_rede"">[id_empresas_rede]</Parameter>"
                };
