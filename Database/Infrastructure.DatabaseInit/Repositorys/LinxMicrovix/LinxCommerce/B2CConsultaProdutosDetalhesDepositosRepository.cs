        ...
        public bool CreateDataTableIfNotExists(string databaseName, string jobName, string untreatedDatabaseName)
        {
            ...
            try
            {
                using (var conn = _conn.GetIDbConnection(databaseName))
                {
                    ...
                    if (result.Count() == 0)
                        conn.CreateTable<Domain.DatabaseInit.Entities.LinxMicrovix.Trusted.LinxCommerce.B2CConsultaProdutosDetalhesDepositos>();
                }
            }
            catch (Exception ex)
            {
            ...
            try
            {
                using (var conn = _conn.GetIDbConnection(untreatedDatabaseName))
                {
                    ...
                    if (result.Count() == 0)
                        conn.CreateTable<Domain.DatabaseInit.Entities.LinxMicrovix.Untreated.LinxCommerce.B2CConsultaProdutosDetalhesDepositos>();
                }
            }
            catch (Exception ex)
            {
        ...
        public async Task<bool> CreateTableMerge(string databaseName, string tableName)
        {
            string? sql = @"IF NOT EXISTS (SELECT * FROM SYS.OBJECTS WHERE TYPE = 'P' AND NAME = 'P_B2CCONSULTAPRODUTOSDETALHESDEPOSITOS_SYNC')
                           ...
                           EXECUTE (
	                           ...
	                           BEGIN
		                           MERGE [LINX_MICROVIX_COMMERCE].[dbo].[B2CCONSULTAPRODUTOSDETALHESDEPOSITOS] AS TARGET
                                   USING [UNTREATED].[dbo].[B2CCONSULTAPRODUTOSDETALHESDEPOSITOS] AS SOURCE
                                   ...
                                   WHEN NOT MATCHED BY TARGET AND SOURCE.[CODIGOPRODUTO] NOT IN (SELECT [CODIGOPRODUTO] FROM [LINX_MICROVIX_COMMERCE].[dbo].[B2CCONSULTAPRODUTOSDETALHESDEPOSITOS]) THEN
			                           INSERT
