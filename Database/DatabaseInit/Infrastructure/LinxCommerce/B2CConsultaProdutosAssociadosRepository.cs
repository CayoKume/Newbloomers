        ...
        public async Task<bool> CreateDataTableIfNotExists(LinxMicrovixJobParameter jobParameter)
        {
            ...
            try
            {
                using (var conn = _conn.GetIDbConnection(jobParameter.databaseName))
                {
                    ...
                    if (result.Count() == 0)
                    {
                        conn.CreateTable<B2CConsultaProdutosAssociados>(tableName: $"{jobParameter.jobName}_raw");
                        conn.CreateTable<B2CConsultaProdutosAssociados>(tableName: $"{jobParameter.jobName}_trusted");
