        ...
        public List<B2CConsultaFormasPagamento?> DeserializeXMLToObject(LinxAPIParam jobParameter, List<Dictionary<string?, string?>> records)
        {
            ...
            for (int i = 0; i < records.Count(); i++)
            {
                try
                {
                    ...
                    {
                        var message = $"Error when convert record - cod_forma_pgto: {records[i].Where(pair => pair.Key == ";
                ...
                catch (Exception ex)
                {
                    throw new GeneralException(
                        message: $"Error when convert record - cod_forma_pgto: {records[i].Where(pair => pair.Key == " - {ex.Message}cod_forma_pgto").Select(pair => pair.Value).FirstOrDefault()} | forma_pgto: {records[i].Where(pair => pair.Key == "forma_pgto").Select(pair => pair.Value).FirstOrDefault()} - {ex.Message}",
