...
namespace Application.LinxMicrovix.Outbound.WebService.Services
{
    public class B2CConsultaTiposCobrancaFreteService : IB2CConsultaTiposCobrancaFreteService
    {
        ...
        private readonly IValidator<Domain.LinxMicrovix.Outbound.WebService.Dtos.LinxCommerce.B2CConsultaTiposCobrancaFrete> _validator;
        ...
        public B2CConsultaTiposCobrancaFreteService(
            ...
            IValidator<Domain.LinxMicrovix.Outbound.WebService.Dtos.LinxCommerce.B2CConsultaTiposCobrancaFrete> validator,
        ...
        public List<B2CConsultaTiposCobrancaFrete?> DeserializeXMLToObject(LinxAPIParam jobParameter, List<Dictionary<string?, string?>> records)
        {
            ...
            for (int i = 0; i < records.Count(); i++)
            {
                try
                {
                    var entity = new Domain.LinxMicrovix.Outbound.WebService.Dtos.LinxCommerce.B2CConsultaTiposCobrancaFrete(
                    ...
                    {
                        var message = $"Error when convert record - codigo_tipo_cobranca_frete: {records[i].Where(pair => pair.Key == "codigo_tipo_cobranca_frete").Select(pair => pair.Value).FirstOrDefault()} | nome_tipo_cobranca_frete: {records[i].Where(pair => pair.Key == "nome_tipo_cobranca_frete").Select(pair => pair.Value).FirstOrDefault()} ";
                    ...
                    list.Add(new B2CConsultaTiposCobrancaFrete(entity, xml));
                }
                catch (Exception ex)
                {
                    throw new GeneralException(
                        message: $"Error when convert record - codigo_tipo_cobranca_frete: {records[i].Where(pair => pair.Key == "codigo_tipo_cobranca_frete").Select(pair => pair.Value).FirstOrDefault()} | nome_tipo_cobranca_frete: {records[i].Where(pair => pair.Key == "nome_tipo_cobranca_frete").Select(pair => pair.Value).FirstOrDefault()} - {ex.Message}",
        ...
        public async Task<bool> GetRecords(LinxAPIParam jobParameter)
        {
            _logger
               .Clear()
               .AddLog(EnumJob.B2CConsultaTiposCobrancaFrete);
            ...
            foreach (var cnpj_emp in cnpjs_emp)
            {
                ...
                if (xmls.Count() > 0)
                {
                    ...
                    if (_b2cConsultaTiposCobrancaFreteCache.Count == 0)
                    {
                        var list_existentes = await _b2cConsultaTiposCobrancaFreteRepository.GetRegistersExists(jobParameter: jobParameter, registros: listRecords);
                        _b2cConsultaTiposCobrancaFreteCache = list_existentes.ToList();
                    }

                    var _listSomenteNovos = listRecords.Where(x => !_b2cConsultaTiposCobrancaFreteCache.Any(y =>
                        y == x.recordKey
                    )).ToList();

                    if (_listSomenteNovos.Count() > 0)
                    {
                        _b2cConsultaTiposCobrancaFreteRepository.BulkInsertIntoTableRaw(records: _listSomenteNovos, jobParameter: jobParameter);
                        ...
                        _b2cConsultaTiposCobrancaFreteCache.AddRange(_listSomenteNovos.Select(x => x.recordKey));
                    }
