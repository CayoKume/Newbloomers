...
namespace Application.LinxMicrovix.Outbound.WebService.Services
{
    public class B2CConsultaProdutosCamposAdicionaisDetalhesService : IB2CConsultaProdutosCamposAdicionaisDetalhesService
    {
        ...
        private readonly IValidator<Domain.LinxMicrovix.Outbound.WebService.Dtos.LinxCommerce.B2CConsultaProdutosCamposAdicionaisDetalhes> _validator;
        private static List<string?> _b2cConsultaProdutosCamposAdicionaisDetalhesCache { get; set; } = new List<string?>();
        ...
        public B2CConsultaProdutosCamposAdicionaisDetalhesService(
            ...
            IValidator<Domain.LinxMicrovix.Outbound.WebService.Dtos.LinxCommerce.B2CConsultaProdutosCamposAdicionaisDetalhes> validator,
        ...
        public List<B2CConsultaProdutosCamposAdicionaisDetalhes?> DeserializeXMLToObject(LinxAPIParam jobParameter, List<Dictionary<string?, string?>> records)
        {
            ...
            for (int i = 0; i < records.Count(); i++)
            {
                try
                {
                    var entity = new Domain.LinxMicrovix.Outbound.WebService.Dtos.LinxCommerce.B2CConsultaProdutosCamposAdicionaisDetalhes(
                    ...
                    {
                        var message = $"Error when convert record - id_campo_detalhe: {records[i].Where(pair => pair.Key == "id_campo_detalhe").Select(pair => pair.Value).FirstOrDefault()} | descricao: {records[i].Where(pair => pair.Key == "descricao").Select(pair => pair.Value).FirstOrDefault()} ";
                    ...
                    list.Add(new B2CConsultaProdutosCamposAdicionaisDetalhes(entity, xml));
                }
                catch (Exception ex)
                {
                    throw new GeneralException(
                        message: $"Error when convert record - id_campo_detalhe: {records[i].Where(pair => pair.Key == "id_campo_detalhe").Select(pair => pair.Value).FirstOrDefault()} | descricao: {records[i].Where(pair => pair.Key == "descricao").Select(pair => pair.Value).FirstOrDefault()} - {ex.Message}",
        ...
        public async Task<bool> GetRecord(LinxAPIParam jobParameter, string? identificador, string? cnpj_emp)
        {
            _logger
               .Clear()
               .AddLog(EnumJob.B2CConsultaProdutosCamposAdicionaisDetalhes);
        ...
        public async Task<bool> GetRecords(LinxAPIParam jobParameter)
        {
            ...
            foreach (var cnpj_emp in cnpjs_emp)
            {
                ...
                if (xmls.Count() > 0)
                {
                    ...
                    if (_b2cConsultaProdutosCamposAdicionaisDetalhesCache.Count == 0)
                    {
                        var list_existentes = await _b2cConsultaProdutosCamposAdicionaisDetalhesRepository.GetRegistersExists(jobParameter: jobParameter, registros: listRecords);
                        _b2cConsultaProdutosCamposAdicionaisDetalhesCache = list_existentes.ToList();
                    }

                    var _listSomenteNovos = listRecords.Where(x => !_b2cConsultaProdutosCamposAdicionaisDetalhesCache.Any(y =>
                        y == x.recordKey
                    )).ToList();

                    if (_listSomenteNovos.Count() > 0)
                    {
                        _b2cConsultaProdutosCamposAdicionaisDetalhesRepository.BulkInsertIntoTableRaw(records: _listSomenteNovos, jobParameter: jobParameter);
                        await _coreRepository.CallDbProcMerge(jobParameter.schema, jobParameter.tableName, _logger.GetExecutionGuid());
                        ...
                        _b2cConsultaProdutosCamposAdicionaisDetalhesCache.AddRange(_listSomenteNovos.Select(x => x.recordKey));
                    }
