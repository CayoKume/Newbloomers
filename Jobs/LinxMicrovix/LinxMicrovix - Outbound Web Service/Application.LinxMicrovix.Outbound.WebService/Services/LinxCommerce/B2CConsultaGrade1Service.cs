        ...
        public List<B2CConsultaGrade1?> DeserializeXMLToObject(LinxAPIParam jobParameter, List<Dictionary<string?, string?>> records)
        {
            ...
            for (int i = 0; i < records.Count(); i++)
            {
                try
                {
                    ...
                    {
                        var message = $"Error when convert record - codigo_grade1: {records[i].Where(pair => pair.Key == ";
                ...
                catch (Exception ex)
                {
                    throw new GeneralException(
                        message: $"Error when convert record - codigo_grade1: {records[i].Where(pair => pair.Key == " - {ex.Message}codigo_grade1").Select(pair => pair.Value).FirstOrDefault()} | nome_grade1: {records[i].Where(pair => pair.Key == "nome_grade1").Select(pair => pair.Value).FirstOrDefault()} - {ex.Message}",
