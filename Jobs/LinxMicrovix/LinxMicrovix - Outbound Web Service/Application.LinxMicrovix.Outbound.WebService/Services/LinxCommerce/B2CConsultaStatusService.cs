            ...
            for (int i = 0; i < records.Count(); i++)
            {
                ...
                catch (Exception ex)
                {
                    throw new GeneralException(
                        message: $"Error when convert record - id_status: {records[i].Where(pair => pair.Key == "id_status").Select(pair => pair.Value).FirstOrDefault()} | descricao_status: {records[i].Where(pair => pair.Key == "descricao_status").Select(pair => pair.Value).FirstOrDefault()} - {ex.Message}",
