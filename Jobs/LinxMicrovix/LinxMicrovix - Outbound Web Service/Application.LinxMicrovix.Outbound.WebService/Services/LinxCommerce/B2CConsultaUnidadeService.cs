...
namespace Application.LinxMicrovix.Outbound.WebService.Services
{
    public class B2CConsultaUnidadeService : IB2CConsultaUnidadeService
    {
        ...
        private readonly IValidator<Domain.LinxMicrovix.Outbound.WebService.Dtos.LinxCommerce.B2CConsultaUnidade> _validator;
        private static List<string?> _b2cConsultaUnidadeCache { get; set; } = new List<string?>();
        ...
        public B2CConsultaUnidadeService(
            ...
            IValidator<Domain.LinxMicrovix.Outbound.WebService.Dtos.LinxCommerce.B2CConsultaUnidade> validator,
        ...
        public List<B2CConsultaUnidade?> DeserializeXMLToObject(LinxAPIParam jobParameter, List<Dictionary<string?, string?>> records)
        {
            ...
            for (int i = 0; i < records.Count(); i++)
            {
                try
                {
                    var entity = new Domain.LinxMicrovix.Outbound.WebService.Dtos.LinxCommerce.B2CConsultaUnidade(
                    ...
                    {
                        var message = $"Error when convert record - unidade: {records[i].Where(pair => pair.Key == "unidade").Select(pair => pair.Value).FirstOrDefault()} | timestamp: {records[i].Where(pair => pair.Key == "timestamp").Select(pair => pair.Value).FirstOrDefault()} ";
                    ...
                    list.Add(new B2CConsultaUnidade(entity, xml));
                }
                catch (Exception ex)
                {
                    throw new GeneralException(
                        message: $"Error when convert record - unidade: {records[i].Where(pair => pair.Key == "unidade").Select(pair => pair.Value).FirstOrDefault()} | timestamp: {records[i].Where(pair => pair.Key == "timestamp").Select(pair => pair.Value).FirstOrDefault()} - {ex.Message}",
        ...
        public async Task<bool> GetRecords(LinxAPIParam jobParameter)
        {
            ...
            foreach (var cnpj_emp in cnpjs_emp)
            {
                ...
                if (xmls.Count() > 0)
                {
                    ...
                    if (_b2cConsultaUnidadeCache.Count == 0)
                    {
                        ...
                        _b2cConsultaUnidadeCache = list_existentes.ToList();
                    }

                    var _listSomenteNovos = listRecords.Where(x => !_b2cConsultaUnidadeCache.Any(y =>
                        y == x.recordKey
                    )).ToList();
                    ...
                    if (_listSomenteNovos.Count() > 0)
                    {
                        ...
                        _b2cConsultaUnidadeCache.AddRange(_listSomenteNovos.Select(x => x.recordKey));
                    }
